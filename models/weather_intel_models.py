...
def _get_home_abbr(team_abbr: str, opp_abbr: str, game_date: str) -> str | None:
    ...
    NOTE: previously matched on team.abbreviation from the schedule
    response, which is only populated when the request hydrates "team" --
    plain team-filtered schedule calls like this one return team objects
    as just {id, name, link}, so abbreviation was always "" and this
    always returned None (silently zeroing wind_out_mph for every NRFI
    read, masked because park_run_factor alone kept the environment dict
    non-empty). Matching on team id instead -- and simply returning
    team_abbr/opp_abbr directly rather than round-tripping through the
    API's abbreviation string at all -- fixes that; see the identical
    bug/fix in data/mlb_probable_pitchers.py,
    core/intelligence/umpire_intel.py, and data/statcast_lineup_platoon.py.
    """
    ...
    opp_id = _MLB_TEAM_IDS.get(opp_abbr.upper())
        ...
        if data:
            for date_block in data.get("dates", []):
                for game in date_block.get("games", []):
                    ...
                    if home_id == team_id and away_id == opp_id:
                        result = team_abbr.upper()
                        break
                    if away_id == team_id and home_id == opp_id:
                        result = opp_abbr.upper()
                        break
                if result:
                    break
