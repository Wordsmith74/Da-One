...
def _wlp_line(records: Iterable[dict[str, Any]]) -> dict[str, Any]:
    ...
    for r in records:
        ...
        if result == "win":
            ...
            continue
        profit += r.get("profit_units", 0.0)
        staked += r.get("stake_units", 1.0)
...
def generate_summary(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    by_confidence = _group_report(
        graded, lambda r: _bucket(float(r.get("confidence_score", -1)), CONFIDENCE_BUCKETS)
    )
    by_edge = _group_report(
        graded, lambda r: _bucket(float(r.get("edge_percentage", -1)), EDGE_BUCKETS)
    )
