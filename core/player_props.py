...
def get_player_prop_candidates(
    ...
    for event in events[:_event_cap]:
        ...
        for (player_name, mkt_key), sides in prop_map.items():
            ...
            _k_bf_splits = None
            if sport_up == "MLB" and mkt_key == "pitcher_strikeouts" and _wl is not None:
                try:
            ...
            for direction, info in sides.items():
                ...
                if _k_bf_splits is not None:
                    try:
                        ...
                        from models.sport_config import MLB as _MLB_AGREEMENT_CFG
                        ...
                        # ── Agreement-fraction gate (sport_config.MLB["min_side_
                        # agreement_frac"], backtest-calibrated to 0.90) ──────────
                        # This is the statistic that actually separates real edge
                        # from projection noise per the resampled-redraw design in
                        # models/monte_carlo.py -- it was being computed and folded
                        # into confidence as a minor +15pt nudge, but never used to
                        # reject a candidate outright. A low agreement_frac means
                        # the outer Monte Carlo redraws don't agree on which side
                        # even has the edge, i.e. exactly the noise this gate was
                        # calibrated to catch. Below threshold, don't publish this
                        # side off the Binomial path -- fall through to the
                        # existing Gaussian/synthetic-history path below instead,
                        # same as when _k_bf_splits is None.
                        _min_agreement = _MLB_AGREEMENT_CFG.get("min_side_agreement_frac", 0.90)
                        if _mc["agreement_frac"] < _min_agreement:
                            logger.debug(
                                f"[player_props] K-prop AGREEMENT REJECT -- "
                                f"{player_name} {direction}: agreement_frac="
                                f"{_mc['agreement_frac']:.2f} < {_min_agreement:.2f}"
                            )
                            _k_prop_precomputed = None
                        else:
                            _implied_prob = (
                            ...
                            # Same 82-cap discipline as _precomp_confidence() in
                            # game_markets.py (precomputed candidates can't
                            # auto-qualify as Diamond without corroboration).
                            # Base 62 (vs. that function's 68) reflects that this
                            # market has no book-count signal folded in the way
                            # moneyline does; real-start count substitutes for
                            # book_boost as the sample-size term instead.
                            _confidence = round(min(82.0, max(50.0,
                                62.0
