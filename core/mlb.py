...
def get_first_inning_runs(game_pk: int) -> Optional[float]:
    """
    Combined (both teams) first-inning runs, from the linescore -- same
    endpoint and pattern as get_f5_total_runs, just innings[:1] instead of
    innings[:5]. Used to grade NRFI (0 runs) / YRFI (1+ runs) picks in
    core/nrfi.py.
    """
    data = _get(f"{BASE_URL}/game/{game_pk}/linescore")
    if not data:
    ...
    if len(innings) < 1:
        logger.warning("mlb_first_inning_incomplete", extra={"game_pk": game_pk})
        return None
    try:
        home_r = innings[0].get("home", {}).get("runs", 0)
        away_r = innings[0].get("away", {}).get("runs", 0)
        if home_r is None or away_r is None:
            # First inning not yet played/recorded (e.g. rain delay before
            # the top of the 1st finished) -- distinct from "confirmed 0".
            logger.warning("mlb_first_inning_not_recorded", extra={"game_pk": game_pk})
            return None
        return float(home_r) + float(away_r)
    except (TypeError, KeyError) as e:
        logger.warning("mlb_first_inning_parse_error", extra={"game_pk": game_pk, "error": str(e)})
        return None


def get_run_line_margin(game_pk: int, side: str) -> Optional[float]:
    """
    Run differential for `side` ("home" or "away"): side_runs - opponent_runs.

    This is a raw fact, not yet combined with a betting line -- same
    division of labor as get_game_total_runs() (fetcher returns the fact,
    core/run_line.py's grade() applies the pick's line/side to decide
    win/push/loss). Kept this way so the fetcher has no knowledge of what
    was bet, matching every other get_* function in this module.
    """
    box = get_boxscore(game_pk)
    if not box:
        return None
    ...
    if side not in ("home", "away"):
        logger.warning("mlb_run_line_bad_side", extra={"game_pk": game_pk, "side": side})
        return None
    other = "away" if side == "home" else "home"
    try:
        side_runs = box["teams"][side]["teamStats"]["batting"]["runs"]
        other_runs = box["teams"][other]["teamStats"]["batting"]["runs"]
        return float(side_runs) - float(other_runs)
    except KeyError as e:
        logger.warning(
            "mlb_run_line_missing_field", extra={"game_pk": game_pk, "error": str(e)}
        )
        return None
