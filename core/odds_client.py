...
# Process-level cache: teamId → list of recent game totals.
# Populated on first fetch; reused for all subsequent candidates in the
# same run without additional API calls.
_MLB_HISTORY_CACHE: dict[int, list[float]] = {}
...
def _mlb_dynamic_game_window() -> int:
    """
...
def get_mlb_game_totals_history(team_name: str, games: int | None = None) -> list[float] | None:
    """
    ...
    *games* defaults to a dynamic window scaled to how deep into the MLB
    ...
    Results are cached at process level so each team is only fetched once
    per engine run regardless of how many game candidates involve that team.
    ...
    if games is None:
        games = _mlb_dynamic_game_window()
    ...
    if team_id in _MLB_HISTORY_CACHE:
        return _MLB_HISTORY_CACHE[team_id]
    ...
    try:
        ...
        if not totals:
            print(
                ...
                f"('{team_name}') — falling back to synthetic history.",
                flush=True,
            )
            return None

        recent = totals[-games:]
        _MLB_HISTORY_CACHE[team_id] = recent
...
def fetch_todays_candidates(sport: str) -> list[dict[str, Any]]:
    """
    ...
    # ── Tier 1: Slate cache ───────────────────────────────────────────────
    cached = read_slate(sport_up, date_str)
    ...
    # ── Tier 2: Live API fetch with ET-bounded time filter ────────────────
    commence_from, commence_to = _et_day_bounds_utc(today_et)
    params = (
        "regions=us,eu&markets=totals&oddsFormat=american&dateFormat=iso"
    ...
    for game in raw_games:
        ...
        # late manual runs never pick up in-play lines.
        if game_et <= now_est():
            print(
            ...
            for mkt in bk.get("markets", []):
                ...
                for outcome in mkt.get("outcomes", []):
                    ...
                    if name == "over":
                        ...
                        book_lines_over.append({"book": bk_title, "line": float(pt)})
                    elif name == "under":
                        under_lines.append((float(pt), int(price), bk_title))
                        book_lines_under.append({"book": bk_title, "line": float(pt)})
        ...
        if sport_up == "MLB":
            home_hist = get_mlb_game_totals_history(home_team)
            away_hist = get_mlb_game_totals_history(away_team)
            ...
            try:
                from core.intelligence.game_logs import get_team_game_totals
                home_hist_rg = get_team_game_totals(sport_up, home_abbr)
                away_hist_rg = get_team_game_totals(sport_up, away_abbr)
    ...
    write_slate(sport_up, candidates, date_str)
