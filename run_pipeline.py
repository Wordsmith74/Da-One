...
def discover_wnba_player_props(games, max_players_per_team=3):
    ...
    for game in games:
        ...
        for team in (home_team, away_team):
            ...
            try:
                players = get_wnba_team_players(team_id, team_name=team_name)
            except Exception as e:
            ...
            for p in players:
                ...
                try:
                    logs = get_wnba_player_game_logs(pid, season, limit=1, player_name=player_name or None)
                except Exception:
                    continue
                if logs and logs[0].get("min", 0) and logs[0]["min"] > 0:
                    candidates.append((pid, logs[0]["min"], player_name))

            candidates.sort(key=lambda t: t[1], reverse=True)
            for pid, _, player_name in candidates[:max_players_per_team]:
                out.append({"player_id": pid, "season": season, "player_name": player_name})
...
def process_mlb_k_prop(raw):
    ...
    print(f"  [debug] {raw.get('pitcher')} shrunk={shrunk_k_pct:.4f} adv={adv_k_pct:.4f} csw={raw.get('csw_pct')} swstr={raw.get('swstr_pct')}")
    ...
    if USE_LIVE_DATA:
        try:
            ...
            def _fetch_wnba_one(t):
                return t["player_id"], live_fetch_wnba_player_prop(t["player_id"], t["season"], player_name=t.get("player_name"))
