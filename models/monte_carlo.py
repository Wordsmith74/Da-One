...
def k_prop_edge_with_uncertainty(k_pct, k_pct_std, bf_mean, bf_std,
                                  market_line, market_odds, side="over",
                                  n_outer=60, n_inner=1500, seed=None):
    """
    ...
    side: "over" or "under" -- which side of the market_line/market_odds
    this call is pricing. Each direction has its own book price in a real
    two-way market, so this must be called once per side rather than
    inferring "the better side" from a single over-priced call; agreement_frac
    is the fraction of the n_outer redraws where THIS side actually shows
    positive edge (not a competition between over/under).
    """
    rng = random.Random(seed)
    edges, agrees = [], []
    for _ in range(n_outer):
        ...
        implied_prob = (100 / (market_odds + 100)) if market_odds > 0 else (abs(market_odds) / (abs(market_odds) + 100))
        side_prob = summary["over_prob"] if side == "over" else summary["under_prob"]
        edge_pct = (side_prob - implied_prob) * 100
        edges.append(edge_pct)
        agrees.append(edge_pct > 0)

    mean_edge_pct = sum(edges) / len(edges)
    agreement_frac = sum(agrees) / len(agrees)
    return {"mean_edge_pct": mean_edge_pct, "agreement_frac": agreement_frac, "side": side}
