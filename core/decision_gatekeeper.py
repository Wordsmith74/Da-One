...
# MLB full-game run line and moneyline therefore have NO calibrated floor of their own
# yet. Both are given the same provisional (4.0, 60.0) floor as a reasoned starting
# point, clearly labeled provisional pending their own graded sample.
...
_MARKET_ENTRY_FLOORS_BY_SPORT: dict[str, dict[str, tuple[float, float]]] = {
    "MLB": {
        "run_line":  (4.0, 60.0),   # full-game run line / spread -- provisional, MLB-only,
                                     # no graded history of its own yet.
                                     # Re-derive from output/threshold_recommendations.json
                                     # once MLB game_class n_graded >= 15.
        "moneyline": (4.0, 60.0),   # full-game moneyline -- newly wired in, provisional,
                                     # aligned to run_line's floor above pending its own
                                     # graded sample (same reasoning as run_line's note).
        "game_total": (2.53, 75.0),  # full-game total -- set from the 2 graded game_total
                                      # picks to date (both wins: COL@SF over 8.5 @ edge
                                      # 3.17%/conf 80.0, LAA@TEX over 7.5 @ edge 2.53%/conf
                                      # 75.0). Edge and confidence floors both set to the
                                      # lower of the two graded picks' own values, so both
                                      # would clear -- n=2 is far below threshold_optimizer.py's
                                      # own 15-graded-pick minimum for a supported threshold.
                                      # Re-derive once MLB game_total has a real sample.
    },
    "WNBA": {
