...
# Market dispatch/reporting alias table. Keys are the *normalized* market
# string. Historical values seen in pick_history.jsonl include display-cased
# forms like "Pitcher Strikeouts" as well as the canonical "pitcher_strikeouts"
# -- both must normalize to the same key or ROI-by-market reports silently
# split one market into two buckets.
_MARKET_ALIAS_MAP = {
    "assists": "player_assists",
    "rebounds": "player_rebounds",
    "strikeouts": "pitcher_strikeouts",
    "player_strikeouts": "pitcher_strikeouts",
    # core/totals.py's grade() already handles both game and F5 totals
    # (see its own docstring), and fetch_actual_stat() in
    # historical_grader.py keys the MLB/WNBA stat fetchers off "totals" --
    # but picks are actually written to pick_history.jsonl with
    # market="game_total". Without this alias "game_total" never matched
    # the "totals" dispatch key, so every game-total pick was rejected
    # with no_grader_for_market and silently never graded.
    "game_total": "totals",
    # shadow_grader.py recovers market names from bet_id strings (e.g.
    # "ATL_total_over_6f0fa1db"), which use the singular "total" --
    # same underlying market as "game_total" above, needs the same alias.
    "total": "totals",
}


def market_normalized(market: str) -> str:
    norm = (market or "").strip().lower().replace(" ", "_")
    return _MARKET_ALIAS_MAP.get(norm, norm)
