...
multiplicative devig). Then blend those per-book fair probabilities using
handicapper-specified weights that differ by market type:

    Player props            : FanDuel 40% / Pinnacle 35% / BetOnline 25%
    Totals / Spread / ML/RL : Pinnacle 40% / BetOnline 35% / FanDuel 25%

If one of the three named books didn't post a two-sided price (missing
entirely, or only quoted one side), its weight is dropped and the remaining
weights are renormalized over whichever of the three actually reported —
this module never silently invents a number for a book that isn't there.
...
# of these before being matched against a weight table, so "BetOnline.ag"
# and "betonline" both resolve to the same weight as "BetOnline".

PLAYER_PROP_WEIGHTS: dict[str, float] = {
    "FanDuel":   0.40,
    "Pinnacle":  0.35,
    "BetOnline": 0.25,
}

GAME_MARKET_WEIGHTS: dict[str, float] = {
    "Pinnacle":  0.40,
    "BetOnline": 0.35,
    "FanDuel":   0.25,
}
...
# If the weight actually covered by reporting books falls below this, the
# result is flagged low_confidence=True rather than silently trusted as a
# real 3-book consensus. Set at "at least one book with the smallest single
# weight in either scheme" (0.25) so a single-book devig doesn't pass as
# equivalent to a full weighted blend.
_MIN_TRUSTED_WEIGHT_COVERAGE = 0.25
...
def get_weights_for_market(market: str) -> dict[str, float]:
    """Pick the player-prop or game-market weighting scheme."""
    from core.market_weights import is_game_market
    try:
        from core.grading_utils import market_normalized
        market = market_normalized(market)
    except Exception:
        pass  # fall back to raw string — is_game_market lowercases anyway
    return GAME_MARKET_WEIGHTS if is_game_market(market) else PLAYER_PROP_WEIGHTS
...
def compute_side_agreement_frac(direction: str, per_book_fair_prob: dict[str, float],
                                 weights: dict[str, float]) -> float | None:
    """
    Fraction of reporting book-weight whose OWN devigged fair probability
    also favors this side (>50%) — i.e. how much of the available
    cross-book evidence agrees this side carries value, not just "how many
    books quoted a price" (that's what devig's weight_covered already
    measures).

    This is the field pick_history.jsonl / shadow_log.jsonl schema calls
    `side_agreement_frac`. It was defined in that schema from an earlier
    pipeline version but never wired up in the current Bayesian/devig
    pipeline (run_pipeline.py never set the key on the pick dict at all,
    so every published pick after the port silently logged it as null).
    This function is the real computation, fed from data devig() already
    produces — no separate model or API call needed.

    Returns None if no books reported (can't measure agreement on nothing),
    matching the None-if-unknown convention used elsewhere in this module
    rather than defaulting to 0.0 or 1.0, either of which would be a
    fabricated result.
    """
    if not per_book_fair_prob:
        return None
    weight_covered = sum(weights.get(b, 0.0) for b in per_book_fair_prob)
    if weight_covered <= 0:
        return None
    agreeing_weight = sum(
        weights.get(b, 0.0) for b, p in per_book_fair_prob.items() if p > 0.5
    )
    return round(agreeing_weight / weight_covered, 4)
...
def weighted_fair_prob_for_candidate(candidate: dict[str, Any]) -> dict[str, Any]:
    ...
    result = get_weighted_fair_prob(
        direction=direction,
        own_book_lines=candidate.get("book_lines", []),
        opposing_book_lines=candidate.get("opposing_book_lines", []),
        weights=weights,
    )
    result["side_agreement_frac"] = compute_side_agreement_frac(
        direction, result.get("per_book_fair_prob", {}), weights
    )
    return result
