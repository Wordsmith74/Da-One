...
# events[] comes back from The Odds API sorted by commence_time ascending,
# so this was silently truncating to only the earliest-starting games of
# the day -- e.g. a global cap of 6 meant MLB (10-15 games/day) never even
# looked at pitchers in afternoon/evening/West Coast games. Now per-sport
# so a paid API tier can raise MLB without over-fetching thin slates like
# WNBA.
_MAX_EVENTS_PER_SPORT_DEFAULT = 6
_MAX_EVENTS_PER_SPORT: dict[str, int] = {
    "MLB":  16,  # covers a full MLB slate (typically 10-15 games/day)
    "WNBA": 8,   # WNBA slates rarely exceed 6-8 games/day
}


def _max_events_for_sport(sport_up: str) -> int:
    return _MAX_EVENTS_PER_SPORT.get(sport_up, _MAX_EVENTS_PER_SPORT_DEFAULT)
...
def get_player_prop_candidates(
    ...
    _event_cap = _max_events_for_sport(sport_up)
    if len(events) > _event_cap:
        logger.warning(
            f"[player_props] {sport_up} slate truncated: {len(events)} event(s) "
            f"available from the Odds API, only processing the first {_event_cap} "
            f"(sorted by commence_time, so these are the earliest-starting games). "
            f"Raise _MAX_EVENTS_PER_SPORT['{sport_up}'] in core/player_props.py if "
            f"this is cutting off real slate coverage."
        )
    else:
        logger.debug(
            f"[player_props] {sport_up}: {len(events)} event(s) available, "
            f"cap is {_event_cap} -- full slate covered."
        )

    for event in events[:_event_cap]:
        ...
        for (player_name, mkt_key), sides in prop_map.items():
            ...
            _workload_confidence_tier = None   # consumed by decision_gatekeeper.py Step 1b4 (OVER hook-risk penalty)
            _wl = None   # bound here so the k/bf splits lookup below is safe even if this try block fails
            if sport_up == "MLB" and mkt_key == "pitcher_strikeouts":
                try:
                    ...
                    # ── Umpire zone-tendency scale (Fix #5, wired for K props) ──
                    # Same core.intelligence.umpire_intel module wired into
                    # odds_client.py for game totals -- currently returns a
                    # zero adjustment for every umpire until
                    # UMPIRE_ZONE_TENDENCY is populated from a real source
                    # (see that module's docstring), but wiring it in now
                    # means the day it IS populated, K-prop projections pick
                    # it up automatically with no further pipeline changes.
                    # Expressed as a multiplicative scale (like every other
                    # layer in this block) rather than odds_client.py's
                    # additive league_mean_adjustment -- reasonable
                    # first-pass conversion once the table has real values:
                    # scale = 1.0 + (adj / expected_ip_baseline_k), but since
                    # the table is empty right now this is a no-op either way.
                    try:
                        from core.odds_client import _abbrev as _mlb_abbrev
                        from core.intelligence.umpire_intel import get_umpire_intel_for_matchup
                        _home_abbr_ump = _mlb_abbrev(home_team)
                        _away_abbr_ump = _mlb_abbrev(away_team)
                        _game_date_ump = (
                            datetime.strptime(_wl.game_date, "%Y-%m-%d").date()
                            if _wl.game_date else None
                        )
                        _ump = get_umpire_intel_for_matchup(
                            _home_abbr_ump, _away_abbr_ump, _game_date_ump,
                        )
                        if _ump.league_mean_adjustment:
                            _ump_scale = 1.0 - (_ump.league_mean_adjustment * 0.02)
                            _orig_u = weighted_proj
                            weighted_proj = round(weighted_proj * _ump_scale, 2)
                            logger.debug(
                                f"[player_props] K umpire scale {player_name}: "
                                f"{_orig_u:.2f} → {weighted_proj:.2f} "
                                f"(×{_ump_scale:.4f}, {_ump.factor_text})"
                            )
                        elif _ump.umpire_name:
                            logger.debug(f"[player_props] {player_name}: {_ump.factor_text}")
                    except Exception as _ump_exc:
                        logger.debug(f"[player_props] umpire_intel unavailable: {_ump_exc}")
            ...
            # ── Real K%/batters-faced splits for pitcher_strikeouts (MLB only) ──
            # Strikeouts are discrete count data bounded by batters faced --
            # a Binomial(bf, k_pct) model, not the Gaussian `hist`/NUTS path
            # below (which, for this market, is itself built from
            # _synthetic_history(hist_mean, ...), i.e. fabricated from the
            # projection rather than real game logs). When enough real
            # recent starts exist, this gets used per-direction below via
            # models.monte_carlo.k_prop_edge_with_uncertainty instead;
            # _k_bf_splits stays None (falling through to the existing
            # Gaussian/synthetic path, unchanged) whenever real data is thin --
            # e.g. a pitcher with < 4 starts this season.
            _k_bf_splits = None
            # Bridged into run_log.json below via the "k_bf_skip_reason" candidate
            # field (see run_pipeline.py) -- get_pitcher_workload() and
            # get_recent_k_bf_splits() only report through the standard `logging`
            # module ("betting_bot" logger), which is a separate sink from
            # run_pipeline.py's RUN_LOG/log(); none of it was reaching
            # output/run_log.json, making every synthetic-data fallback here
            # indistinguishable from an actual data problem (2026-07-28 fix).
            _k_bf_skip_reason: str | None = None
            if sport_up == "MLB" and mkt_key == "pitcher_strikeouts":
                if _wl is None:
                    _k_bf_skip_reason = "pitcher_workload lookup failed (_wl is None)"
                elif not _wl.pitcher_id:
                    _k_bf_skip_reason = "no pitcher_id resolved for this start (schedule lookup miss)"
                else:
                    try:
                        from core.pitcher_workload import get_recent_k_bf_splits
                        _k_bf_season = (
                            int(_wl.game_date[:4]) if _wl.game_date
                            else datetime.now(timezone.utc).year
                        )
                        _k_bf_splits = get_recent_k_bf_splits(_wl.pitcher_id, _k_bf_season)
                        if _k_bf_splits is None:
                            _k_bf_skip_reason = (
                                "get_recent_k_bf_splits() returned None "
                                "(< 6 usable real starts this season)"
                            )
                    except Exception as _kbf_exc:
                        logger.debug(
                            f"[player_props] k/bf splits fetch failed for "
                            f"{player_name}: {_kbf_exc}"
                        )
                        _k_bf_splits = None
                        _k_bf_skip_reason = f"get_recent_k_bf_splits() raised: {_kbf_exc}"

                # AUDIT (2026-08-05): this market used to fall through to the
                # Gaussian/NUTS path below whenever _k_bf_splits was None --
                # and for pitcher_strikeouts that path builds its "historical
                # data" from _synthetic_history(hist_mean, ...), i.e. random
                # noise sampled around the model's own projection, then runs
                # Bayesian inference on that fabricated data to produce a
                # confidence score. Graded results confirmed this was doing
                # real damage: confidence 70-80% and 80-90% picks both won
                # ~51% (should be ~70-90% if the score meant anything), while
                # the live run_log showed the "confidence" number coming
                # almost entirely from a flat +5-13pt market-tier bonus, not
                # from the (frequently 0.00%) computed edge. A miscalibrated-
                # but-confident probability estimate then got fed straight
                # into kelly_stake() in run_pipeline.py, which is exactly how
                # a marginal/break-even raw win rate (52.5%) turned into an
                # 83% max-drawdown bankroll simulation.
                #
                # Strikeouts are discrete count data bounded by batters faced
                # -- Binomial(bf, k_pct) is the correct model, and it's the
                # ONLY one used now for this market. No real recent-start data
                # (_k_bf_splits is None, e.g. < 6 usable starts -- see
                # pitcher_workload._MIN_STARTS) means NO PICK for either
                # direction, full stop. Admitting "we don't have a real
                # signal yet" is strictly better than publishing a confident-
                # looking number backed by invented data.
                if _k_bf_skip_reason is not None:
                    logger.info(
                        f"[player_props] NO PLAY: {player_name} pitcher_strikeouts — "
                        f"{_k_bf_skip_reason} (Gaussian/synthetic fallback disabled "
                        "for this market; real Binomial K%/BF data required)"
                    )
                    continue
            ...
            #
            # AUDIT (2026-08-05): same fix as pitcher_strikeouts, generalized --
            # the problem was never "Gaussian vs. Binomial," it was fabricating
            # "history" from the model's own projection and treating that
            # invented data as evidence for a confidence score. Rebounds/assists
            # aren't Binomial-shaped (no batters-faced-style bounded trial
            # count), so the fix here isn't switching distribution family, it's
            # the same principle as strikeouts: real empirical data or no pick,
            # never synthetic. Threshold raised 5 -> 8 games (5 recent games is
            # a thin empirical std to trust for sizing a stake off of), and the
            # synthetic fallback below the threshold is removed -- previously it
            # silently produced a confident-looking candidate from fabricated
            # data exactly like the strikeouts bug did.
            _WNBA_MIN_REAL_GAMES = 8
            if sport_up == "WNBA":
                ...
                if len(_real_logs) >= _WNBA_MIN_REAL_GAMES:
                    ...
                    logger.info(
                        f"[player_props] NO PLAY: {player_name} {mkt_key} — only "
                        f"{len(_real_logs)} real game log(s) (<{_WNBA_MIN_REAL_GAMES}); "
                        "synthetic Gaussian fallback disabled, real per-game data required"
                    )
                    continue
            else:
            ...
            for direction, info in sides.items():
                ...
                # ── Binomial K-prop edge (real data only; see _k_bf_splits above) ──
                # Prices THIS direction's own line/odds against the empirical
                # k_pct/batters-faced distribution, rather than the Gaussian
                # NUTS/synthetic-history path used by every other market
                # (and used as the fallback here too, when _k_bf_splits is None).
                _k_prop_precomputed = None
                if _k_bf_splits is not None:
                    try:
                        from models.monte_carlo import k_prop_edge_with_uncertainty
                        _mc = k_prop_edge_with_uncertainty(
                            k_pct       = _k_bf_splits["k_pct_mean"],
                            k_pct_std   = _k_bf_splits["k_pct_std"],
                            bf_mean     = _k_bf_splits["bf_mean"],
                            bf_std      = _k_bf_splits["bf_std"],
                            market_line = line,
                            market_odds = odds,
                            side        = direction,
                            seed        = int(seed_val) + (0 if direction == "over" else 1),
                        )
                        _implied_prob = (
                        ...
                        _model_prob = max(
                            0.01, min(0.99, _implied_prob + _mc["mean_edge_pct"] / 100.0)
                        )
                        # AUDIT (2026-08-05): base/cap pulled down from 62/82
                        # to 55/75. The old constants were never fit to
                        # anything -- they were picked to roughly mirror
                        # _precomp_confidence() in game_markets.py, a
                        # different market with a book-count signal this one
                        # doesn't have. There is not yet enough graded
                        # history under the Binomial-only gate (this file's
                        # NO PLAY change above) to refit these from data, so
                        # treat 55/75 as a deliberately conservative holding
                        # value, not a calibrated one. Once
                        # output/pick_history.jsonl has enough
                        # pitcher_strikeouts rows generated *after* this
                        # change, run core/calibration.py's
                        # generate_summary()/roi_by_confidence_bucket on just
                        # this market and refit base/slope/cap so a given
                        # confidence number actually tracks observed win
                        # rate -- do not hand-tune these again without that
                        # check.
                        _confidence = round(min(75.0, max(50.0,
                            55.0
                            + _mc["agreement_frac"] * 15.0
                            + min(5.0, _k_bf_splits["n_starts"] * 0.5)
                        )), 1)
                        _k_prop_precomputed = {
                            "edge":       _mc["mean_edge_pct"],
                            "confidence": _confidence,
                            "model_prob": _model_prob,
                            "n_starts":   _k_bf_splits["n_starts"],
                        }
                    except Exception as _mc_exc:
                        logger.debug(
                            f"[player_props] Binomial K-prop edge failed for "
                            f"{player_name} {direction}: {_mc_exc}"
                        )
                        _k_prop_precomputed = None
                ...
                candidates.append({
                    ...
                    "k_bf_skip_reason":        _k_bf_skip_reason,
                    "context":                 "regular",
                    ...
                    # Workload risk (consumed by decision_gatekeeper.py Step 1b4 — K OVER hook-risk penalty)
                ...
                # Wire in the real Binomial K-prop edge, if computed above --
                # mirrors how moneyline/spread candidates in game_markets.py
                # carry precomputed_edge/confidence/model_prob to bypass the
                # NUTS sampler in run_pipeline.py. Left unset (falls through
                # to the existing historical_data/league_mean Gaussian path,
                # completely unchanged) whenever _k_prop_precomputed is None --
                # i.e. not this market, or fewer than 4 real starts available.
                if _k_prop_precomputed is not None:
                    candidates[-1]["precomputed_edge"]       = _k_prop_precomputed["edge"]
