...
from core.probability_calibrator import calibrate_probability
...
def _derive_bet_params(sim, candidate):
    ...
    raw_confidence = round(min(99.0, 50.0 + z * 25.0), 1)

    # ── Market-agreement damping ────────────────────────────────────────────
    # raw_confidence above comes ONLY from projection-distance (z-score vs
    # the raw sportsbook line) -- it never looks at the devigged market
    # price, so it measures model conviction, not exploitable value. A pick
    # can score high confidence purely because the posterior sits far from
    # the line, even when the devigged market has already priced in exactly
    # that lean (raw_edge ~0). Audit of graded pitcher_strikeouts shadow-log
    # data (2026-08-04) found this decoupling produces a real win-rate dip
    # concentrated in one confidence band: 88% of picks landing there carried
    # ~0% edge (vs ~50% baseline in neighboring bands), and that zero-edge
    # subset won only 36% -- market-blind confidence was inflating exactly
    # the picks with no real value behind them.
    #
    # Damp confidence toward raw_edge's own floor: raw_edge <= 0 (market
    # already fully agrees/disagrees with the model) gets the full haircut;
    # raw_edge >= floor gets none. Linear ramp in between.
    #
    # Floor values -- backtested against graded pitcher_strikeouts shadow-log
    # data (2026-08-04) by replaying this formula over the historical
    # population and checking both (a) the confidence-bucket win-rate curve
    # and (b) volume/win-rate at the market's actual entry gate:
    #   - The first version of this fix borrowed 8.0 (edge_calibrator's
    #     full-calibration-onset floor for props) without evidence it was
    #     the right number for THIS purpose. Backtest showed it was too
    #     aggressive -- it stripped 18 legitimate winners (66.7% win rate)
    #     off the entry-gate-passing population along with the noise it was
    #     meant to remove (174 -> 158 picks, 57.5% -> 55.7% win).
    #   - 2.0-4.0 all backtested equivalently at the gate (176 picks, 56.8%
    #     win, +2/-0 vs the old formula) while still cleaning up the
    #     confidence-bucket dip (65-70 band: ~68-69% win at every value in
    #     this range, vs 37.6% before the fix). 3.0 was chosen from within
    #     that flat range because it matches this market's own under-side
    #     entry floor (_MARKET_ENTRY_FLOORS_BY_SIDE) rather than being
    #     picked arbitrarily.
    #   - Only pitcher_strikeouts has been backtested this way so far. This
    #     same floor is applied to every prop market for now (props were
    #     previously undifferentiated at 8.0 too) -- WNBA player_assists /
    #     player_rebounds should get their own backtest pass before this
    #     value is trusted there. Game markets are untouched (10.0), pending
    #     the same evidence-gathering.
    _damp_floor = 10.0 if is_game_market(mkt) else 3.0
    _damp_min = 0.5
    agreement_ratio = max(0.0, min(1.0, raw_edge / _damp_floor))
    market_damp = _damp_min + (1.0 - _damp_min) * agreement_ratio
    confidence = round(max(0.0, raw_confidence * market_damp), 1)
...
def run_sport_pipeline(sport, as_of_date=None):
    ...
    try:
        # MLB's full-game total was removed from scope 2026-07-10 (see
        # core/market_gate.py) -- skip the fetch entirely rather than pull
        # candidates that would just be blocked downstream. WNBA still uses
        # this same function for its own game_total market, so the skip is
        # scoped to MLB only, not a blanket change to fetch_todays_candidates.
        candidates = [] if sport.upper() == "MLB" else fetch_todays_candidates(sport, as_of_date=as_of_date)
    except Exception as exc:
    ...
    for c in candidates:
        ...
        # ── Surface why MLB pitcher_strikeouts fell back to synthetic data ──
        # (see core/player_props.py "k_bf_skip_reason" comment) -- without this,
        # the synthetic-fallback path is indistinguishable in run_log.json from
        # a genuine stability rejection driven by real data.
        if c.get("k_bf_skip_reason"):
            log("info", sport, f"{bet_id}: using synthetic Gaussian fallback -- {c['k_bf_skip_reason']}")
    ...
    for bet in approved:
        ...
        pick = {
            ...
            "away_team": c.get("away_team", ""),
            "home_team": c.get("home_team", ""),
            # Fix #6 prerequisite: bookmaker_source was computed upstream
            # (odds_client.py / player_props.py both set it on the
            # candidate dict) but never carried through to the published
            # pick dict, so it never reached pick_history.jsonl -- the
            # same "computed but dropped before publication" pattern the
            # side_agreement_frac bug was. Without this, no per-book
            # exposure/concentration tracking is possible after the fact.
        ...
        # ── CALIBRATED PROBABILITY (Fix #2) ──────────────────────────────
        # model_prob (0-100 here) is CCS/Bayesian model *output*, not a
        # calibrated probability -- composite_confidence_score.py's five
        # factors use fixed hand-tuned weights and edge_calibrator's
        # calibrate_edge() is a fixed linear rescale, neither fit to
        # realized outcomes. Audit of pick_history.jsonl showed MLB
        # pitcher_strikeouts picks at 70-90% "confidence" actually won at
        # ~51% -- the scale was simply wrong, and every downstream
        # consumer (Kelly staking above all) was sizing off that wrong
        # number. core/probability_calibrator.py fits an isotonic mapping
        # from raw model_prob -> empirical win rate per (sport,
        # market_class) group, using graded history, and here we apply it
        # before anything sizes a bet off of it. Falls back to raw_prob
        # unchanged when a group doesn't have enough graded picks yet to
        # fit (see probability_calibrator.MIN_SAMPLE_SIZE) -- calibration
        # can only correct what it has evidence for, so an engine with
        # little graded history behaves exactly as it did before this fix.
        _raw_prob_0to1 = (model_prob / 100.0) if model_prob is not None else 0.0
        _calibrated_prob_0to1 = calibrate_probability(
            raw_prob=_raw_prob_0to1, sport=sport, market=bet.market,
        )
        pick["model_prob_raw"] = round(_raw_prob_0to1, 4)
        pick["model_prob_calibrated"] = round(_calibrated_prob_0to1, 4)
        ...
        # size tracks the win-probability estimate rather than the raw/
        # calibrated edge number -- backtesting showed edge magnitude does
        # NOT track actual win rate monotonically in this engine's
        # history, so it should never drive bet size.
        #
        # AUDIT (Fix #2): this now uses the CALIBRATED probability, not
        # raw model_prob -- fractional Kelly only protects the bankroll
        # when the probability fed into it is accurate. Feeding it an
        # overconfident raw model_prob (as before) meant quarter-Kelly was
        # still sizing off a bad number; it dampened the damage but didn't
        # fix the cause. This is the actual fix upstream of the
        # _MAX_STAKE_PCT_BANKROLL backstop below, not a replacement for it
        # -- keep both.
        _kelly_fraction = (MLB if sport == "MLB" else WNBA)["kelly_fraction"]
        _stake_frac = kelly_stake(
            model_prob=_calibrated_prob_0to1,
            american_odds=bet.american_odds,
            kelly_fraction=_kelly_fraction,
        )

        # AUDIT (2026-08-05): hard cap added as a backstop independent of
        # calibration quality. Fractional Kelly only protects you if the
        # win-probability fed into it is accurate -- when model_prob is
        # overconfident (which graded pitcher_strikeouts picks showed it
        # was: 70-90% "confidence" tracked a ~51% actual win rate), quarter-
        # Kelly on a bad probability estimate still produces oversized
        # stakes. Backtesting pick_history.jsonl's actual stake_pct_bankroll
        # values (avg 13.7%, max 25%) through a compounding bankroll
        # simulation showed an 83% max drawdown despite roughly break-even
        # per-bet ROI -- sizing, not just edge, was independently capable of
        # doing the damage. This cap doesn't fix a bad probability estimate,
        # it just stops one bad estimate from being able to blow up the
        # bankroll on its own. 5% is still aggressive for single-game player
        # props; tighten further once roi_by_confidence_bucket in
        # core/calibration.py shows the model_prob feeding kelly_stake() is
        # actually calibrated.
        _MAX_STAKE_PCT_BANKROLL = 5.0
        pick["stake_pct_bankroll"] = round(
            min(_stake_frac * 100, _MAX_STAKE_PCT_BANKROLL), 2
        )
...
def _apply_daily_caps(picks):
    ...
    max_picks_per_day). A sport's cap can be set to None to publish every
    pick that cleared the gatekeeper, uncapped -- MLB is currently set
    this way (was 6/day). Group picks are still edge-sorted below even
    when uncapped, purely for readable ordering in the output, not to
    enforce a cutoff.
    """
