...
Approved markets (normalized internal keys):
    MLB  : pitcher_strikeouts, first_5_total, first_5_ml, first_5_rl, nrfi, yrfi, game_total, run_line
    WNBA : player_assists, player_rebounds, moneyline, game_total
...
Priority order (per spec):
    ...
    8  MLB  nrfi
    9  MLB  yrfi
    10 MLB  game_total   -- full-game total; lower priority than the F5-scoped
                             markets above since F5 is this pipeline's primary
                             MLB total product (see core/market_gate.py)
    11 WNBA game_total   -- same rationale as MLB game_total
    12 MLB  run_line     -- full-game run line; new market, no graded history yet
"""
...
MARKET_PRIORITY: dict[tuple[str, str], int] = {
    ...
    ("MLB",  "game_total"):         10,
    ("WNBA", "game_total"):         11,
    ("MLB",  "run_line"):           12,  # full-game run line -- new market, lowest MLB priority
                                          # until it has graded history to compete on
}
