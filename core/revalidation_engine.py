...
def _recompute_edge(
    ...
    Devigs against *candidate*'s book_lines/opposing_book_lines (same
    weighted multi-book blend used at publish time in run_pipeline.py) when
    a fresh candidate is available and at least one named book quoted both
    sides. Falls back to the raw single-book implied prob otherwise (market
    disappeared, or no book match) -- same behavior as before this change.
    """
    ...
    if candidate:
        try:
            ...
            if devig_result["fair_prob"] is not None:
                return round((model - devig_result["fair_prob"]) * 100.0, 2)
        except Exception as exc:
            logger.debug(f"[revalidation] devig recompute failed, falling back: {exc}")
...
def _send_edge_warning_alert(change: dict[str, Any]) -> bool:
    ...
    text = (
        f"EDGE WARNING — SIGNIFICANT EDGE DECAY\n"
        f"\n"
        f"Pick: {pick_desc}\n"
        f"\n"
    ...
    sent = send_alert(text, source="revalidation_engine.edge_warning", level="warning")
    if sent:
        logger.info(
            f"[revalidation] EDGE WARNING recorded for "
            f"{change.get('bet_id','?')} "
...
def _send_edge_reversal_alert(change: dict[str, Any]) -> bool:
    ...
    sent = send_alert(text, source="revalidation_engine.edge_reversal", level="critical")
    if sent:
        logger.info(
            f"[revalidation] EDGE REVERSAL recorded for "
            f"{change.get('bet_id','?')} "
            f"({opening_edge:.1f}% → {current_edge:.1f}%)"
        )
    return sent
...
def run_revalidation(
    ...
    for row in rows:
        ...
        if candidate:
            ...
            current_edge  = _recompute_edge(current_odds, opening_conf, candidate)
        else:
        ...
        if is_prop and player:
            ...
            if prop_c:
                ...
                current_edge  = _recompute_edge(current_odds, opening_conf, prop_c)
                logger.debug(
