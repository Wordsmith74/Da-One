...
def check_locked_conflict(
    ...
    *,
    ...
    Parameters
    ----------
    skip : When True (replay mode), bypasses the live `results.db` read
           entirely and returns "clear" unconditionally. A replay run has
           no business reading or being influenced by live locked picks —
           there's no "locked" state in a backtest, and hitting the live
           DB from a replay loop would also risk file-lock contention with
           a concurrently running live pipeline.
