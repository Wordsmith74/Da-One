...
PRIOR_WEIGHT_NRFI_GAMES = MLB["prior_weight_nrfi_games"]
...
def shrink_mlb_nrfi_lambda(recent_first_inning_runs_per_team_game, n_games,
                            league_combined_lambda_baseline,
                            prior_weight_games=PRIOR_WEIGHT_NRFI_GAMES,
                            known_shift_event=False):
    """
    Shrinks a TEAM's recent first-inning runs-per-game rate toward the
    league-wide combined first-inning lambda baseline
    (sport_config.MLB['nrfi_combined_lambda_baseline'], split per team as
    baseline/2 by the caller before passing in here).

    Mirrors shrink_mlb_f5_runs' mechanism, but shrinks toward the LEAGUE
    baseline rather than the team's own season average -- unlike F5 scoring
    (where a team's own season total-runs average is a meaningful, fairly
    stable personal prior), first-inning-only samples are small enough per
    team-game (0, 1, or rarely 2+ runs) that even a full season of a team's
    own first-inning history is a thinner signal than the league base rate
    itself. This is exactly the doc's own instruction: "Any model output
    should be sanity-checked against this base rate; large deviations need
    a real causal reason ... not noise."

    recent_first_inning_runs_per_team_game: recent first-inning runs/game
        rate for ONE team (not combined).
    n_games: number of games in the recent sample.
    league_combined_lambda_baseline: the combined (both teams) league prior,
        e.g. MLB['nrfi_combined_lambda_baseline'] -- caller divides by 2 for
        a per-team prior before calling, since this function shrinks one
        team's rate at a time.
    known_shift_event: True for a confirmed non-noise cause (e.g. a new
        starter with a real first-inning track record already established
        elsewhere, a permanent lineup change) -- discounts the prior so the
        recent sample dominates.
    """
    if n_games <= 0:
        return league_combined_lambda_baseline

    weight = prior_weight_games * KNOWN_SHIFT_EVENT_PRIOR_DISCOUNT if known_shift_event else prior_weight_games

    shrunk = (
        (recent_first_inning_runs_per_team_game * n_games + league_combined_lambda_baseline * weight)
        / (n_games + weight)
    )
    return max(0.02, shrunk)  # floor -- a team's true first-inning rate is never exactly 0
