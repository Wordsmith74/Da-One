...
def _format_change_alert(
    ...
    lines.append(
        "View the full change log in the slate history."
    )
    return "\n".join(lines)


def _send_telegram(text: str) -> None:
    # Renamed in spirit only (kept the function name to avoid touching every
    # call site) -- this no longer talks to Telegram. See core/alerts.py.
    from core.alerts import send_alert
    send_alert(text, source="slate_versioner", level="warning")
