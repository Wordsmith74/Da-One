...
# Fix #7 (quarantine pre-fix edge_pct data): edge_pct values written before
# calibrate_edge() was fixed are raw/uncalibrated (24-50% "edges" logged
# directly -- see README's original patch notes) and are not comparable to
# post-fix calibrated values (1-15% range). Confirmed by inspecting
# pick_history.jsonl directly: mean |edge_pct| runs 29-42% for picks
# generated 2026-06-30 through 2026-07-02, then drops and stays in the
# single-to-low-double-digit range (matching the documented calibrated
# scale) from 2026-07-03 onward -- a clean, visible cutover, not a gradual
# drift. Any edge-threshold search that includes both eras is comparing
# apples to a completely different unit; a "1.2% edge" cutoff computed
# across both eras is meaningless because "1.2%" didn't mean the same
# thing before and after the fix. Both optimize() and walk_forward_validate()
# exclude picks generated before this cutover from their edge/confidence
# search entirely, rather than silently blending eras.
EDGE_PCT_TRUSTED_SINCE = "2026-07-03T00:00:00+00:00"
...
def optimize(pick_history_path: str = DEFAULT_PICK_HISTORY_PATH) -> dict[str, Any]:
    ...
    n_before_quarantine = len(graded)
    graded = [r for r in graded if (r.get("generated_at") or "") >= EDGE_PCT_TRUSTED_SINCE]
    n_quarantined = n_before_quarantine - len(graded)
    ...
    payload = {
        ...
        "n_graded": len(graded),
        "n_quarantined_pre_fix": n_quarantined,
        "edge_pct_trusted_since": EDGE_PCT_TRUSTED_SINCE,
...
def _chronological_split(recs: list[dict], train_frac: float = 0.6) -> tuple[list[dict], list[dict]]:
    """
    Split records chronologically (by generated_at) into an earlier TRAIN
    fold and a later TEST fold.

    Deliberately NOT a random split. optimize() searches for the cutoffs
    that maximize ROI on whatever data it's given -- if that search set
    included picks from both before and after some later-evaluated pick
    (a random split would do this), the "out-of-sample" test would still
    leak future information into thresholds tuned against the past. A
    chronological split is the only split that mirrors how the thresholds
    will actually be used going forward: fit on what's already happened,
    applied to what hasn't happened yet.
    """
    def _ts(r: dict) -> str:
        return r.get("generated_at") or ""
    ordered = sorted(recs, key=_ts)
    cut = max(1, int(len(ordered) * train_frac))
    return ordered[:cut], ordered[cut:]


def walk_forward_validate(pick_history_path: str = DEFAULT_PICK_HISTORY_PATH,
                           train_frac: float = 0.6) -> dict[str, Any]:
    """
    Out-of-sample check for optimize()'s recommendations (Fix #3).

    optimize() searches pick_history.jsonl for the edge/confidence cutoffs
    that maximize realized ROI, then reports that same in-sample ROI as
    the expected performance of those cutoffs -- but the cutoffs were
    chosen BECAUSE they looked good on that exact data, so the reported
    ROI is optimistic by construction. There was previously no train/test
    split anywhere in this codebase to catch that.

    This function instead:
      1. Splits each (sport, market_class) group's graded picks
         chronologically into an earlier TRAIN fold and a later TEST fold.
      2. Runs the same threshold search optimize() uses, but ONLY on
         TRAIN.
      3. Applies the resulting TRAIN-fit cutoffs to the untouched TEST
         fold and reports the TEST-fold ROI -- an honest estimate of what
         those thresholds would have done on picks they were never tuned
         against.
      4. A tier is "validated" only if its test-fold ROI still clears
         GOLD_ROI_FLOOR with enough test-fold volume to mean something.
         Otherwise it's "failed_walk_forward": the in-sample number in
         optimize()'s output for that group should NOT be trusted as a
         real edge -- it's more likely a threshold fit to noise in the
         training window.

    Run this ALONGSIDE optimize(), not instead of it -- optimize()'s full
    in-sample scan is still useful for exploration; this is the gate
    before any of its recommendations should actually change
    decision_gatekeeper.SPORT_TIER_THRESHOLDS.
    """
    records = read_jsonl(pick_history_path)
    graded = [r for r in records if r.get("actual_result") in ("win", "loss", "push")]
    # Fix #7: same pre-fix edge_pct quarantine as optimize() -- see
    # EDGE_PCT_TRUSTED_SINCE docstring above.
    graded = [r for r in graded if (r.get("generated_at") or "") >= EDGE_PCT_TRUSTED_SINCE]
    ...
    # Test folds are smaller than the full group by construction (train_frac
    # of the data goes to train) -- require less volume there to call
    # something validated, but still a real floor, not zero.
    min_test_sample_size = max(8, MIN_SAMPLE_SIZE // 2)

    group_reports: list[dict[str, Any]] = []
    for (sport, market_class), recs in sorted(groups.items()):
        n = len(recs)
        # Need enough graded picks to produce BOTH a trainable train fold
        # and an evaluable test fold -- not just enough for one or the
        # other.
        if n < MIN_SAMPLE_SIZE + min_test_sample_size:
            group_reports.append({
                "sport": sport, "market_class": market_class, "n_graded": n,
                "status": "insufficient_data_for_walk_forward",
                "note": f"Need >= {MIN_SAMPLE_SIZE + min_test_sample_size} graded picks to "
                        f"split into a trainable train fold and an evaluable test fold "
                        f"(have {n}).",
            })
            continue

        train, test = _chronological_split(recs, train_frac)
        train_window = (train[0]["generated_at"], train[-1]["generated_at"]) if train else (None, None)
        test_window = (test[0]["generated_at"], test[-1]["generated_at"]) if test else (None, None)

        edge_candidates = _quantile_points([r["_edge_abs"] for r in train], N_CANDIDATES)
        conf_candidates = _quantile_points([r["_confidence"] for r in train], N_CANDIDATES)
        cons_values = [r["_consensus"] for r in train if r["_consensus"] is not None]
        consensus_evaluated = len(cons_values) >= MIN_SAMPLE_SIZE
        cons_candidates: list[Optional[float]] = (
            [None] + _quantile_points(cons_values, N_CANDIDATES) if consensus_evaluated else [None]
        )

        train_tiers = _peel_tiers(train, edge_candidates, conf_candidates, cons_candidates)

        tier_results: dict[str, dict[str, Any]] = {}
        for tier_name, cand in train_tiers.items():
            if cand is None:
                tier_results[tier_name] = {"status": "no_train_candidate"}
                continue
            # Apply the TRAIN-fit cutoff, unmodified, to the untouched TEST fold.
            test_cand = _score(
                test, cand.edge_threshold, cand.confidence_threshold, cand.consensus_threshold,
            )
            if test_cand is None or test_cand.n < min_test_sample_size:
                tier_results[tier_name] = {
                    "status": "insufficient_test_volume",
                    "train": asdict(cand),
                    "test_n": test_cand.n if test_cand else 0,
                    "note": f"Only {(test_cand.n if test_cand else 0)} test-fold picks cleared "
                            f"this cutoff (need >= {min_test_sample_size}) -- can't validate "
                            f"either way.",
                }
                continue
            validated = test_cand.roi >= GOLD_ROI_FLOOR
            tier_results[tier_name] = {
                "status": "validated" if validated else "failed_walk_forward",
                "train": asdict(cand),
                "test": asdict(test_cand),
                "roi_drop": round(cand.roi - test_cand.roi, 4),
                "note": (
                    f"Train ROI {cand.roi:+.1%} -> Test ROI {test_cand.roi:+.1%} on "
                    f"n={test_cand.n} picks the search never saw."
                    if validated else
                    f"FAILED: train ROI {cand.roi:+.1%} did not hold up out-of-sample "
                    f"(test ROI {test_cand.roi:+.1%} on n={test_cand.n}) -- likely "
                    f"overfit to the training window. Do not deploy this cutoff."
                ),
            }
    ...
    return {
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "method": f"chronological_split walk-forward (train_frac={train_frac})",
        "min_sample_size_train": MIN_SAMPLE_SIZE,
        "min_sample_size_test": min_test_sample_size,
        "groups": group_reports,
    }


def print_walk_forward_report(payload: dict[str, Any]) -> None:
    print(f"\n=== Walk-Forward Validation ({payload['method']}) ===")
    for g in payload["groups"]:
        print(f"\n{g['sport']} / {g['market_class']}  (n_graded={g['n_graded']})")
        if g["status"] != "ok":
            print(f"  {g['status']}: {g.get('note', '')}")
            continue
        print(f"  train_window={g['train_window']}  test_window={g['test_window']}  "
              f"(train_n={g['train_n']}, test_n={g['test_n']})")
        for tier_name in ("Gold", "Diamond", "Nuke"):
            tr = g["tiers"].get(tier_name)
            if tr is None:
                continue
            print(f"  {tier_name:8s} -> {tr['status']}: {tr.get('note', '')}")
...
DEFAULT_WALK_FORWARD_OUTPUT_PATH = "output/walk_forward_validation.json"
...
def main() -> None:
    ...
    parser.add_argument("--walk-forward-output", default=DEFAULT_WALK_FORWARD_OUTPUT_PATH)
    parser.add_argument("--train-frac", type=float, default=0.6,
                         help="Fraction of each group's graded history (chronologically "
                              "earliest first) used to fit thresholds before testing them "
                              "on the remaining, later fraction.")
    parser.add_argument("--skip-walk-forward", action="store_true",
                         help="Only run the in-sample optimize() scan -- skip the "
                              "out-of-sample gate. Not recommended; the in-sample numbers "
                              "alone overstate expected performance.")
    ...
    print(f"(full in-sample detail written to {args.output})")

    if args.skip_walk_forward:
        return

    wf_payload = walk_forward_validate(args.pick_history, train_frac=args.train_frac)
    ...
    print_walk_forward_report(wf_payload)
    print(f"\n(full walk-forward detail written to {args.walk_forward_output})")
    print(
        "\nOnly deploy tiers marked 'validated' above to "
        "decision_gatekeeper.SPORT_TIER_THRESHOLDS -- 'failed_walk_forward' means the "
        "in-sample recommendation in threshold_recommendations.json for that tier did not "
        "hold up on picks it wasn't fit to, and is more likely noise than real edge."
    )
