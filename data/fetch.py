...
# used everywhere else in this engine. Confirmed live 2026-07: ESPN's
# site API 400s on ".../teams/OAK/..." -- the Athletics' current ESPN
# slug is "ATH" (RotoWire's site made the same change). Also confirmed
# live 2026-07 against ESPN's own team listings: the White Sox (MLB) and
# four WNBA franchises carry the same kind of mismatch. Kept in sync with
# the matching table in core.data_fetcher (not deduped into one shared
# constant, same as the rest of this module vs. that one -- see this
# module's docstring) because get_espn_team_roster() and
# get_espn_team_injuries() below build their own URLs directly rather
# than going through fetch_espn().
...
def _espn_team_slug(team_id):
    if isinstance(team_id, str):
        return _ESPN_TEAM_ABBR_ALIAS.get(team_id.upper(), team_id)
    return team_id
...
def get_espn_team_roster(sport, league, team_id):
    ...
    """
    url = f"https://site.api.espn.com/apis/site/v2/sports/{sport}/{league}/teams/{_espn_team_slug(team_id)}/roster"
    r = requests.get(url, timeout=15)
    r.raise_for_status()
    return r.json()
...
def get_espn_team_injuries(sport, league, team_id):
    """
    Same confirmation status as get_espn_team_roster -- documented, not
    separately verified.

    ESPN's site API has been intermittently returning 500s for individual
    teams (confirmed in run logs -- e.g. CIN, CHC, KC, COL, LAD all 500'd
    on one call, then several of those same teams succeeded moments
    later), which looks like a transient server-side hiccup rather than
    anything wrong with the URL or team abbreviation. A bare
    requests.get() with no retry turns each of those into a hard failure
    for the whole call, even though the very next attempt usually
    succeeds. Retry a couple of times with a short backoff before giving
    up -- but only for 5xx responses; a 4xx (bad team slug, etc.) won't
    be fixed by retrying so it fails immediately instead of wasting time.
    """
    url = f"https://site.api.espn.com/apis/site/v2/sports/{sport}/{league}/teams/{_espn_team_slug(team_id)}/injuries"
    last_exc = None
...
# Per-process cache keyed by team_abbr -- run logs show the same team's
# injuries being fetched twice back-to-back (once for the "over" market,
# once for "under"), which doubled the number of RotoWire/ESPN calls and
# doubled exposure to any transient ESPN failure for no benefit, since
# injury status doesn't change within a single pipeline run. None results
# are cached too (a real "no data" for this run), so a team that fails
# both sources isn't retried a second time only to fail the same way.
_MLB_INJURY_CACHE: dict[str, dict | None] = {}
_WNBA_INJURY_CACHE: dict[str, dict | None] = {}


def get_wnba_team_injuries(team_abbr, roster_names=None):
    """
    WNBA injury fetch: tries the FREE RotoWire injury-news scrape first
    (see data/rotowire_injuries_wnba.py -- no API key, no login), and
    falls back to ESPN's get_espn_team_injuries() if RotoWire is
    unreachable, its markup changed, or it has no recent news for this
    team.

    ESPN's site API takes team abbreviations directly in the URL path,
    not a separate numeric team_id (confirmed live via the MLB path --
    see get_mlb_team_injuries() below), so team_abbr is passed straight
    through here too.

    Previously this called data.rotowire_injuries.get_team_injuries(),
    which depended on a core.wnba_stats_client module that doesn't exist
    in this codebase -- that import always failed, so this path always
    returned None regardless of ESPN's health. Switched to
    rotowire_injuries_wnba, which doesn't need a roster lookup at all
    (RotoWire's news feed already tags each entry with its team).

    The ESPN fallback below used to just print a warning and return None
    -- not an actual fallback. Fixed to actually call ESPN.

    roster_names is accepted for backward compatibility with existing
    callers but is not used (no longer needed -- see above).

    Returns the same {"injuries": [...]} shape either way -- callers
    (models/injury_intel.py) don't need to know which source answered.
    """
    team_abbr = team_abbr.upper()
    if team_abbr in _WNBA_INJURY_CACHE:
        return _WNBA_INJURY_CACHE[team_abbr]

    from data.rotowire_injuries_wnba import get_recent_wnba_injuries as _rotowire_get

    result = None
    try:
        data = _rotowire_get(team_abbr)
        if data is not None:
            result = data
    ...
    if result is None:
        try:
            data = get_espn_team_injuries("basketball", "wnba", team_abbr)
            # ESPN returns a real (if empty) dict for a team with zero
            # currently-listed injuries -- e.g. {} or {"injuries": []}.
            # That's valid data, not a failure: injury_intel.py already
            # does `injury_data.get("injuries") or []`, so an empty dict
            # flows through safely as "confirmed no injuries" rather than
            # needing to be treated as "source unavailable". Checking
            # `is not None` instead of plain truthiness is what makes
            # that distinction -- a bare `if data:` would silently
            # discard a legitimate empty-dict response and misreport a
            # healthy team as a fetch failure.
            if data is not None:
                if not data:
                    print(f"  [info] ESPN WNBA injuries ({team_abbr}): got an empty response body -- treating as \"confirmed no injuries\", not a failure")
                result = data
        except Exception as exc:
            print(f"  [warn] ESPN fallback WNBA injury fetch also failed for {team_abbr}: {exc}")

    _WNBA_INJURY_CACHE[team_abbr] = result
    return result


def get_mlb_team_injuries(team_abbr):
    """
    MLB injury fetch: tries the FREE RotoWire injury-news scrape first
    (see data/rotowire_injuries_mlb.py -- no API key, no login), and
    falls back to ESPN's get_espn_team_injuries() if RotoWire is
    unreachable, its markup changed, or it has no recent news for this
    team.

    ESPN's site API takes team abbreviations directly in the URL path
    (e.g. ".../teams/ATL/injuries"), not a separate numeric team_id, so
    team_abbr is passed straight through -- no id lookup needed. (ESPN's
    "OAK" -> "ATH" rename is handled centrally in
    core.data_fetcher.fetch_espn(), not here, since it affects schedule/
    stats calls too, not just injuries.)

    Previously this branch only printed a warning and returned None --
    i.e. it was not actually a fallback, so any team RotoWire couldn't
    resolve (e.g. no recent injury news to scrape) had NO injury source
    at all. Fixed to actually call ESPN.

    Returns the same {"injuries": [...]} shape either way.
    """
    team_abbr = team_abbr.upper()
    if team_abbr in _MLB_INJURY_CACHE:
        return _MLB_INJURY_CACHE[team_abbr]

    from data.rotowire_injuries_mlb import get_recent_mlb_injuries as _rotowire_mlb_get

    result = None
    try:
        data = _rotowire_mlb_get(team_abbr)
        if data is not None:
            result = data
    except Exception as exc:
        print(f"  [warn] RotoWire (free) MLB injury fetch failed for {team_abbr}: {exc} -- falling back to ESPN")

    if result is None:
        try:
            data = get_espn_team_injuries("baseball", "mlb", team_abbr)
            # See get_wnba_team_injuries() above for why `is not None`
            # (not plain truthiness) is the right check here.
            if data is not None:
                if not data:
                    print(f"  [info] ESPN MLB injuries ({team_abbr}): got an empty response body -- treating as \"confirmed no injuries\", not a failure")
                result = data
        except Exception as exc:
            print(f"  [warn] ESPN fallback MLB injury fetch also failed for {team_abbr}: {exc}")

    _MLB_INJURY_CACHE[team_abbr] = result
    return result
...
def get_mlb_team_k_rate_allowed(team_id, before_date, n_games=10, lookback_days=30):
    """
    Returns team_id's batters' season K% (strikeouts / plate-appearances, 0-1
    float) from Baseball Savant via pybaseball. Used to remove the
    opponent-quality confound from a pitcher's recent K sample BEFORE Bayesian
    shrinkage runs (see models.bayesian.adjust_recent_ks_for_opponent).

    Source: pybaseball.batting_stats() pulls from FanGraphs/Statcast --
    free, no API key, no rate limit. One call fetches the whole league so
    we only hit the network once per run (cached in _SAVANT_TEAM_K_CACHE).

    team_id is a balldontlie integer ID. We map it to a FanGraphs team
    abbreviation via BDL_ID_TO_FG_ABBREV; if the ID is missing from the map
    (e.g. a newly added team or a bad lookup), we return None rather than
    guessing -- callers must treat None as "skip the adjustment".
    """
    # BDL integer team ID -> FanGraphs/pybaseball team abbreviation.
    # Source: balldontlie /mlb/v1/teams list cross-referenced with FanGraphs.
    # Add any missing IDs here (print the BDL team list to find them).
    BDL_ID_TO_FG_ABBREV = {
        1:  "ARI", 2:  "ATL", 3:  "BAL", 4:  "BOS", 5:  "CHC",
        6:  "CWS", 7:  "CIN", 8:  "CLE", 9:  "COL", 10: "DET",
        11: "HOU", 12: "KC",  13: "LAA", 14: "LAD", 15: "MIA",
        16: "MIL", 17: "MIN", 18: "NYM", 19: "NYY", 20: "OAK",
        21: "PHI", 22: "PIT", 23: "SD",  24: "SF",  25: "SEA",
        26: "STL", 27: "TB",  28: "TEX", 29: "TOR", 30: "WSH",
    }

    fg_abbrev = BDL_ID_TO_FG_ABBREV.get(team_id)
    if fg_abbrev is None:
        print(f"  [warn] get_mlb_team_k_rate_allowed: BDL team_id {team_id} not in "
              f"BDL_ID_TO_FG_ABBREV map -- add it to data/fetch.py; returning None "
              f"(opponent K-rate adjustment will be skipped, not guessed)")
        return None

    # Cache the whole-league batting stats for this run -- pybaseball makes
    # one HTTP call per season to FanGraphs; no reason to repeat it per team.
    global _SAVANT_TEAM_K_CACHE
    season = int(before_date[:4])

    if _SAVANT_TEAM_K_CACHE.get("season") != season:
        try:
            from pybaseball import batting_stats
            df = batting_stats(season, qual=0)   # qual=0 = all players, no PA floor
            # Aggregate to team level -- sum K and PA across all batters on that team
            df["_k_pct_k"]  = df.get("SO", df.get("K", 0)).fillna(0)
            df["_k_pct_pa"] = df.get("PA", 0).fillna(0)
            team_totals = (
                df.groupby("Team")[["_k_pct_k", "_k_pct_pa"]]
                .sum()
                .reset_index()
            )
            team_totals["k_pct"] = team_totals.apply(
                lambda r: r["_k_pct_k"] / r["_k_pct_pa"] if r["_k_pct_pa"] > 0 else None,
                axis=1,
            )
            _SAVANT_TEAM_K_CACHE = {
                "season": season,
                "data": dict(zip(team_totals["Team"], team_totals["k_pct"])),
            }
            print(f"  [info] Savant team K% cache loaded for {season}: "
                  f"{len(_SAVANT_TEAM_K_CACHE['data'])} teams")
        except ImportError:
            print("  [warn] pybaseball not installed -- opponent K-rate adjustment skipped "
                  "(add pybaseball to requirements.txt to enable)")
            _SAVANT_TEAM_K_CACHE = {"season": season, "data": {}}
        except Exception as e:
            print(f"  [warn] get_mlb_team_k_rate_allowed: pybaseball fetch failed "
                  f"({type(e).__name__}: {e}) -- returning None")
            _SAVANT_TEAM_K_CACHE = {"season": season, "data": {}}

    k_pct = _SAVANT_TEAM_K_CACHE["data"].get(fg_abbrev)
    if k_pct is None:
        print(f"  [warn] get_mlb_team_k_rate_allowed: no K% found for {fg_abbrev} "
              f"in Savant data -- returning None (opponent K-rate adjustment skipped)")
    return k_pct


_SAVANT_TEAM_K_CACHE: dict = {}
