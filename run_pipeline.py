...
from core.edge_calibrator import is_game_market
...
def _enrich_integrity_fields(c, sport):
    ...
    Mutates `c` in place. Best-effort: every intelligence call already has
    its own internal fail-safe (returns a zeroed dataclass on error), so a
    bad fetch here just means a field stays unset, not a pipeline crash.
    """
    ...
    try:
        ...
        if injury.factor_text or injury.injury_count:
            c.setdefault("injury_report", injury.factor_text or f"{injury.injury_count} injuries")
            c["injury_score"] = injury.impact_score
    ...
    try:
        ...
        if rest.rest_days is not None or rest.travel_miles is not None:
            c["rest_days"] = rest.rest_days
            c["travel_factor"] = rest.travel_miles
    except Exception as exc:
    ...
    try:
        ...
        if venue.park_factor is not None:
            c["park_factor"] = venue.park_factor
        # Rotation projection isn't produced by any standalone module yet --
        # reuse lineup intel as a proxy signal (it already reflects who's in
        # the rotation via injury/availability data) rather than leaving it
        # permanently unset.
        c.setdefault("rotation_score", c.get("injury_score", 0.0))
    except Exception as exc:
        log("warn", sport, f"{c.get('bet_id', '?')}: venue_intel enrichment failed -- {exc}")

    # Market agreement score -- no dedicated scorer exists yet anywhere in
    # the repo (composite_confidence_score.py just reads it with a default
    # of 50 if missing). Set the same neutral default explicitly here so the
    # integrity filter sees a present-but-neutral value instead of "missing".
