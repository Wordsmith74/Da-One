...
class SimulationEngine:
    ...
    def analyze(
        ...
        # Bug fix: posterior["posterior_std"] previously stayed at the
        # PRE-floor (and, in playoff mode, pre-compression) value even
        # though mc_std -- the floored/compressed value -- is what
        # run_monte_carlo() actually simulated with. Every downstream
        # consumer of sim["posterior"]["posterior_std"] (run_pipeline.py's
        # check_stability() gate, relative_sigma_pct, and
        # _derive_bet_params()'s confidence z-score) read the raw value
        # instead, so a collapsed/synthetic posterior could pass the
        # stability gate as artificially "ELITE" stable while its actual
        # probabilities were already computed off a wider, floor-corrected
        # std. Store the raw value separately for debugging and make
        # posterior_std reflect what was actually simulated.
        posterior["posterior_std_raw"] = posterior["posterior_std"]
        posterior["posterior_std"] = mc_std
