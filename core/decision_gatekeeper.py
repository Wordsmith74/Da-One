...
#
# NOTE: pitcher_strikeouts / first_5_total / first_5_ml / first_5_rl / nrfi / yrfi
# are currently unreachable -- MLB scope was narrowed to moneyline / run_line /
# game_total only (see core/market_gate.py's ALLOWED_MARKETS["MLB"]), so no
# candidate for these markets reaches this function anymore. Kept (not deleted)
# in case that scope is ever widened back, same convention as WNBA's
# "team_total" entry below.
...
_MARKET_ENTRY_FLOORS_BY_SPORT: dict[str, dict[str, tuple[float, float]]] = {
    "MLB": {
        ...
        # "game_total": no explicit floor -- falls back to the universal
        # SPORT_TIER_THRESHOLDS["MLB"] gate below, same as before this change.
    },
    "WNBA": {
