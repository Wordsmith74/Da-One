...
def get_wnba_team_injuries(team_abbr, roster_names=None):
    ...
    callers but is no longer used: data.rotowire_injuries.get_team_injuries()
    resolves the team roster itself (via core.wnba_stats_client) to filter
    the league-wide RotoWire page down to one team, so it only takes
    team_abbr. Passing roster_names through used to raise
    "get_team_injuries() got an unexpected keyword argument 'roster_names'".
