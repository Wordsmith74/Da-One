...
def get_mlb_team_k_rate_allowed(team_id, before_date, n_games=10, lookback_days=30):
    """
    Returns team_id's batters' K rate (strikeouts / plate-appearances, 0-1
    float) over recent completed games, looking backward from *before_date*.
    Used to remove the opponent-quality confound from a pitcher's recent K
    sample BEFORE Bayesian shrinkage runs (see
    models.bayesian.adjust_recent_ks_for_opponent) -- shrinkage corrects for
    sample-size noise, not for whether the recent starts happened to be
    against unusually punchout-prone or contact-heavy lineups.

    NOT live-verified: balldontlie's MLB API does not have a documented
    team-level batting endpoint as of this writing. This assumes
    /mlb/v1/stats accepts team_ids[]/dates[]/stat_type=batting params and
    each row has "strikeouts" and either "plate_appearances" or "at_bats".
    If any of that is wrong, this returns None rather than guessing --
    print(stats) once against a live response and fix the field names here
    if so. Callers MUST treat None as "skip the adjustment", never as 0 or
    league average (silently injecting a wrong number is worse than no
    adjustment at all).
    """
    from datetime import datetime, timedelta
    ...
    while games_counted < n_games and days_checked < lookback_days:
        cursor -= timedelta(days=1)
        days_checked += 1
        date_str = cursor.strftime("%Y-%m-%d")
        try:
            stats = _bdl_get(
                "/mlb/v1/stats",
                params={"team_ids[]": team_id, "dates[]": date_str, "stat_type": "batting"},
            ).get("data", [])
        except Exception:
            continue
        if not stats:
            continue
        for row in stats:
            total_k += row.get("strikeouts", 0) or 0
            total_pa += row.get("plate_appearances", row.get("at_bats", 0)) or 0
        games_counted += 1

    if total_pa == 0:
        print(f"  [warn] get_mlb_team_k_rate_allowed: no batting stats resolved for team "
              f"{team_id} in the {lookback_days} days before {before_date} -- check "
              f"/mlb/v1/stats field names against a live response; returning None "
              f"(opponent K-rate adjustment will be skipped, not guessed)")
        return None

    return total_k / total_pa
