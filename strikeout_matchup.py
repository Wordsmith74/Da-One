...
_LEAGUE_SWSTR     = 0.250   # league-average WHIFF% (swings-and-misses / total swings).
                              # NOTE: this constant is intentionally named to match the
                              # `whiff_pct` field below, which is populated directly from
                              # Savant's `whiff_percent` column (~24-26% league-wide) --
                              # NOT swinging-strike rate (SwStr%, whiffs / total pitches,
                              # ~11% league-wide), despite the similar names. Layer 5 below
                              # divides whiff_pct by this constant, so the two MUST use the
                              # same denominator (swings) or every pitcher gets a bogus ~2x
                              # ratio. Fixed 2026-06-30: this was previously set to 0.115
                              # (the SwStr% baseline) while still being divided into Savant's
                              # whiff_percent (true Whiff%), which inflated Layer 5's scale
                              # toward the clamp ceiling for nearly every pitcher and was the
                              # primary driver of a systematic "over" bias in K-prop picks.
