...
_MARKET_ENTRY_FLOORS: dict[str, tuple[float, float]] = {
    "pitcher_strikeouts": (2.0, 68.0),   # MLB Ks — reverted to Gold bar (68) per user request.
                                          # NOTE: graded history showed conf≥78 at 77.8% (25-8)
                                          # vs conf<78 at 29.4% (5-12) — the 78 floor was backed
                                          # by real data. At 68, Gold-band Ks (68-78 conf) will
                                          # pass through again despite that losing record.
    ...
    "moneyline":          (0.0, 68.0),   # WNBA ML (alternate key)
    "h2h":                (0.0, 68.0),   # WNBA ML — market_normalized("h2h") → "h2h"
    "team_total":         (0.0, 68.0),   # WNBA team total — newly wired, no graded history yet
    "run_line":           (0.0, 68.0),   # WNBA spread (game_markets.py labels all spreads "run_line") — newly wired, no graded history yet
}
...
def evaluate_tier(edge_percentage: float, confidence_score: float, sport: str = _DEFAULT_SPORT_KEY) -> Tier | None:
    """
...
def stamp_tier(bet: Bet, sport: str = _DEFAULT_SPORT_KEY) -> Bet:
    ...
    """
    bet.tier = evaluate_tier(bet.edge_percentage, bet.confidence_score, sport)
    return bet
...
def check_for_conflicts(list_of_bets: list[Bet], sport: str = _DEFAULT_SPORT_KEY) -> list[Bet]:
    ...
    for i in range(n):
        for j in range(i + 1, n):
            ...
            lower.tier = evaluate_tier(lower.edge_percentage, lower.confidence_score, sport)
    ...
    for (_gid, _mn), _grp in _gm_groups.items():
        ...
        for _loser in _all_sides:
            ...
            _loser.tier = evaluate_tier(_loser.edge_percentage, _loser.confidence_score, sport)
    ...
    for game_bets in by_game.values():
        ...
        for tt in team_totals:
            ...
            if ratio >= _PROP_CONSENSUS_THRESHOLD:
                ...
                tt.tier = evaluate_tier(tt.edge_percentage, tt.confidence_score, sport)
...
def run_gatekeeper(bets: list[Bet], sport: str = _DEFAULT_SPORT_KEY) -> dict[str, list[Bet]]:
    ...
    # ── Step 1b4: MLB strikeout OVER hook-risk penalty ────────────────────────
    # Backtest finding (see pick_history.jsonl, MLB pitcher_strikeouts):
    # UNDER K props won 73% (8-3) vs OVER K props at 55% (6-5). Losing UNDERs
    # missed narrowly (actual 0.5-1.5 K over the line -- model was in the right
    # neighborhood). Losing OVERs missed badly (actual 0.5-4.5 K UNDER the line
    # -- signature of a short/rough start: quick hook, early trouble, no
    # extension). A short outing helps UNDER and hurts OVER -- the risk is
    # asymmetric, but the workload-scaled mean projection (player_props.py)
    # is symmetric. This step adds the missing asymmetric penalty using the
    # workload model's own confidence tier as the short-outing risk proxy.
    #
    # Applies ONLY to MLB pitcher_strikeouts OVER picks.
    #   workload_confidence_tier == "Low"    → −10 confidence
    #   workload_confidence_tier == "Medium" → −5  confidence
    #   "High" or missing (no workload data) → no penalty
    _K_HOOK_RISK_PENALTY: dict[str, float] = {"Low": 10.0, "Medium": 5.0}
    ...
    for bet in bets:
        ...
        tier_label = bet.raw_result.get("workload_confidence_tier")
        penalty    = _K_HOOK_RISK_PENALTY.get(tier_label, 0.0)
        if penalty <= 0:
            continue
        ...
        bet.confidence_score = max(0.0, bet.confidence_score - penalty)
        reason = (
            f"K hook-risk [workload={tier_label}]: OVER penalized for "
            f"short-outing risk — conf −{penalty:.0f}pts "
            f"({raw_conf:.1f}→{bet.confidence_score:.1f})"
        )
        bet.flag_reason = _append_flag(bet.flag_reason, reason)
        bet.tier        = evaluate_tier(bet.edge_percentage, bet.confidence_score, sport)
        if bet.tier is None:
            bet.flagged = True
        logger.debug(f"[gatekeeper] K-HOOK-RISK  {bet.bet_id}: {reason}")
    ...
    # ── Step 1.4c: WNBA player-prop tier rescue ──────────────────────────────
    # No graded WNBA prop history exists yet to justify a strikeout-style
    # calibrated floor (pending backtest.py sample). Until then, treat a hard
    # 68-conf cliff the same way we used to treat it for Ks: a 65.4-conf,
    # real-edge pick (see Jonquel Jones near-miss, run_log 2026-07-03) is a
    # sweet-spot problem, not a quality problem. Re-admit WNBA player props
    # rejected by evaluate_tier() at conf ≥ 65 and edge ≥ 1.0 at Gold Standard.
    # Revisit once WNBA props have a graded sample (see calibration.py).
    _WNBA_PROP_RESCUE_CONF = 65.0
    _WNBA_PROP_RESCUE_EDGE = 1.0
    _WNBA_RESCUE_MKTS = frozenset({"player_points", "player_rebounds", "player_assists"})
    ...
    for bet in bets:
        ...
        if sport.upper() != "WNBA":
            continue
        mkt = market_normalized(bet.market)
        if mkt not in _WNBA_RESCUE_MKTS:
            continue
        if (bet.confidence_score >= _WNBA_PROP_RESCUE_CONF
                and bet.edge_percentage >= _WNBA_PROP_RESCUE_EDGE):
            bet.tier    = Tier.GOLD
            bet.flagged = False
            bet.flag_reason = None
            logger.debug(
                f"[gatekeeper] WNBA PROP RESCUE  {bet.bet_id}: "
                f"edge={bet.edge_percentage:.2f}%  conf={bet.confidence_score:.1f} "
                f"→ re-admitted at Gold Standard (WNBA prop conf floor=65, provisional)"
            )
    ...
    # WNBA props use 65, matching the Step 1.4c rescue floor above — a
    # separate, higher broadcast floor would just re-reject picks the rescue
    # step just admitted (see Jonquel Jones: rescued at 65.4, then rejected
    # again by the old 75 floor). One number, one gate, same pattern as
    # strikeouts. All other props use the standard floor (75).
    _PROP_CONF_FLOOR          = 75.0
    _STRIKEOUT_PROP_CONF_FLOOR = 68.0
    _WNBA_PROP_CONF_FLOOR      = 65.0
    ...
    for bet in bets:
        ...
        if "strikeouts" in mkt:
            floor = _STRIKEOUT_PROP_CONF_FLOOR
        elif sport.upper() == "WNBA":
            floor = _WNBA_PROP_CONF_FLOOR
        else:
    ...
    # We require +2 confidence points above the tier floor for any plus-money
    # bet to ensure genuine model conviction. (Was +3; WNBA NYL/CHI moneylines
    # missed the old 81 Diamond floor by 0.5-0.6pts at conf 80.4/80.5 —
    # penalizing conviction that tight isn't the discipline we want. +2 keeps
    # a real buffer above the tier floor without punishing a rounding error.)
    #
    # Floors (tier base + 2):
    #   Nuke:         85 → 87  (plus-money)
    #   Diamond:      78 → 80  (plus-money)
    #   Gold Standard:68 → 70  (plus-money)
