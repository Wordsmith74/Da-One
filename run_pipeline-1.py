...
def discover_mlb_probable_pitchers():
    ...
    for event in events:
        ...
        for side_c, side_label in ((home_c, "home"), (away_c, "away")):
            ...
            cached_id = lookup_player_id(id_cache, pitcher_name)
            if cached_id is not None:
                out.append({
                    "pitcher_id": cached_id,
                    "pitcher_name": pitcher_name,
                    "home_name": home_name,
                    "away_name": away_name,
                })
                remember_player_id(id_cache, pitcher_name, cached_id)  # refresh last_seen
                cache_dirty = True
                continue
            ...
            try:
                ...
                continue
            if not matches:
                # Full "First Last" search came back empty -- before giving up, retry with
                # just the last name. balldontlie's search behavior across multi-word names
                # isn't fully documented, and a last-name-only query is much more likely to
                # hit for common cases (it just risks more candidates for common surnames,
                # which the len(matches) > 1 check below already warns about).
                last_name = pitcher_name.strip().split()[-1]
                try:
                    matches = get_mlb_player_search(last_name)
                except Exception as e:
                    print(f"  [warn] balldontlie player search (last-name fallback) failed for "
                          f"'{pitcher_name}': {e} -- skipping")
                    continue
                if matches:
                    print(f"  [info] '{pitcher_name}' resolved via last-name fallback ('{last_name}')")
            if not matches:
            ...
            best = matches[0]
            if len(matches) > 1:
                # Common surname (or a last-name-only fallback search) can return several
                # players -- prefer one whose first name actually matches before just
                # taking the first result.
                first_name = pitcher_name.strip().split()[0].lower()
                exact = [m for m in matches if str(m.get("first_name", "")).lower() == first_name]
                if len(exact) == 1:
                    best = exact[0]
                else:
            ...
            out.append({
                "pitcher_id": best.get("id"),
                "pitcher_name": pitcher_name,
                "home_name": home_name,
                "away_name": away_name,
            })
            remember_player_id(id_cache, pitcher_name, best.get("id"))
            cache_dirty = True
