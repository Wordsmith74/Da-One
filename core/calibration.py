...
from .grading_utils import market_normalized
...
def generate_summary(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    by_market = _group_report(graded, lambda r: market_normalized(r.get("market", "unknown")))
    # `tier` is stamped by decision_gatekeeper.py and persisted to
    # pick_history.jsonl by data/cache_history.py:append_picks(). Rows
    # written before that fix will still show "unknown" -- only new picks
    # going forward will carry a real tier.
    by_tier = _group_report(graded, lambda r: r.get("tier") or "unknown")  # "Nuke"/"Diamond"/"Gold Standard"
    by_confidence = _group_report(
        graded, lambda r: _bucket(float(r.get("confidence", -1)), CONFIDENCE_BUCKETS)
    )
    by_edge = _group_report(
        graded, lambda r: _bucket(abs(float(r.get("edge_pct", -1))), EDGE_BUCKETS)
    )
