...
def get_wnba_team_injuries(team_abbr, roster_names=None):
    ...
    (see data/rotowire_injuries_wnba.py -- no API key, no login), and only
    ...
    (RotoWire's news feed already tags each entry with its team), same
    pattern as the MLB path in get_mlb_team_injuries() below.

    roster_names is accepted for backward compatibility with existing
    callers but is not used (see above -- no longer needed).
...
def get_mlb_team_injuries(team_abbr):
    ...
    (see data/rotowire_injuries_mlb.py -- no API key, no login), and only
    falls back to ESPN's get_espn_team_injuries() if RotoWire is
    unreachable or its markup changed.

    Same pattern as get_wnba_team_injuries() above, adapted for MLB:
    RotoWire's news feed doesn't need a roster lookup to filter by team
    (each entry already carries its team name), so this is simpler than
    the WNBA path.
    ...
    print(f"  [warn] No free RotoWire MLB news for {team_abbr}; ESPN fallback requires team_id "
          f"-- call get_espn_team_injuries('baseball', 'mlb', team_id) directly if needed.")
    return None
