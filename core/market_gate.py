...
ALLOWED_MARKETS: dict[str, frozenset[str]] = {
    # MLB scope: moneyline / run line (spread) / game total / pitcher strikeouts.
    # F5 total/ML/RL and NRFI/YRFI remain out of scope -- any candidate for
    # those markets is blocked here exactly like an out-of-scope NBA candidate
    # would be. pitcher_strikeouts uses its own tier thresholds/entry floor
    # (see core/decision_gatekeeper.py's _MARKET_TIER_THRESHOLDS /
    # _MARKET_ENTRY_FLOORS) rather than the sport-wide default.
    "MLB": frozenset({
        "game_total",           # Full-game total     (odds_client.py fetch_todays_candidates)
        "moneyline",            # Full-game moneyline  (precomputed, core/game_markets.py._process_moneyline)
        "run_line",             # Full-game run line   (precomputed) -- MLB has no separate
                                 # "spread" market from the run line; "rl"/"spread"/"spreads"
                                 # all alias to this one key, see MARKET_ALIASES below.
