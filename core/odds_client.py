...
def fetch_todays_candidates(
    ...
    for game in raw_games:
        ...
        if sport_up == "MLB":
            ...
            # Fix #5: umpire zone-tendency adjustment. Currently returns a
            # zero adjustment for every game until
            # core.intelligence.umpire_intel.UMPIRE_ZONE_TENDENCY is
            # populated from a real source -- see that module's docstring.
            # Wired in now (rather than left unintegrated) so the moment
            # that table is populated, the adjustment flows through
            # automatically with no further pipeline changes needed.
            _ump_factor = None
            try:
                from core.intelligence.umpire_intel import get_umpire_intel_for_matchup
                _ump_factor = get_umpire_intel_for_matchup(home_abbr, away_abbr, game_et.date())
                _pitching_adj += _ump_factor.league_mean_adjustment
            except Exception as exc:
                print(f"[odds_client] umpire_intel unavailable: {exc}", flush=True)
            ...
            if _pitching_adj:
                ...
                print(
                    ...
                    f"ump={getattr(_ump_factor, 'league_mean_adjustment', 0.0):+.2f}  "
