...
#   MLB  — no prop markets in scope. Pipeline now focuses on MLB moneyline /
#          run line / game total only; pitcher_strikeouts removed from scope
#          on request (same empty-string pattern as NBA below).
...
_PROP_MARKETS: dict[str, str] = {
    ...
    "MLB":  "",   # blocked: no prop markets in scope
}
...
_PROPLINE_MARKETS: dict[str, list[str]] = {
    "WNBA": ["player_rebounds", "player_assists"],
    "MLB":  [],   # no prop markets in scope (pitcher_strikeouts removed on request)
...
def _wnba_prop_projection(
    ...
    as_of_date: str | None = None,
) -> tuple[float, float, float | None, float | None, float, float, str, str, str, str, str]:
    """
    ...
    Returns:
      (weighted_proj, season_avg, l5_avg, l10_avg,
       ...
       blowout_level, reallocation_note, matchup_note)
    """
    ...
    return (final_proj, season_avg, l5_avg, l10_avg,
            projected_minutes, minutes_range, minutes_stability, role_label,
            _blowout_level, _realloc_note, _matchup_note)
