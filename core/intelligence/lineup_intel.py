...
from core.data_fetcher import fetch_espn, fetch_wnba_injuries, fetch_mlb_injuries  # Rule 1+2 — fetch with timeout + waterfall
...
class LineupIntelFactor:
    ...
    # Structured per-player detail (position + severity), added so downstream
    # consumers (e.g. core/intelligence/wnba_opp_intel.py's teammate-out
    # reallocation layer) can reason about WHICH position is missing instead
    # of just how many players are hurt. `injuries` above stays untouched for
    # existing callers (run_pipeline.py's factor_text display).
    injuries_detail: list[dict] = field(default_factory=list)
...
def _compute(team_abbr: str, sport: str, bet_on_this_team: bool) -> LineupIntelFactor:
    ...
    # WNBA and MLB both try their FREE RotoWire scrape first (off-ESPN
    # diversification); other sports keep the original ESPN-primary waterfall.
    if sport.upper() == "WNBA":
        result = fetch_wnba_injuries(team_abbr)
    elif sport.upper() == "MLB":
        result = fetch_mlb_injuries(team_abbr)
    else:
    ...
    for inj in injuries_raw:
        ...
        if score > 0:
            ...
            injury_detail.append({
                "name": name, "position": position.upper(),
                "status": status, "severity": sev, "impact_mult": mult,
            })
