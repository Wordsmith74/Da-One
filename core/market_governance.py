...
Approved markets (normalized internal keys):
    MLB  : moneyline, run_line, game_total
           -- scope narrowed on request. pitcher_strikeouts, first_5_total,
              first_5_ml, first_5_rl, nrfi, yrfi are intentionally OUT of scope
              (see core/market_gate.py).
    WNBA : player_assists, player_rebounds, moneyline, game_total  (unchanged)

Priority order (per spec):
    1  MLB  moneyline    -- full-game moneyline; newly in scope
...
PUBLICATION_MARKETS: dict[str, frozenset[str]] = {
    # Narrowed to moneyline / run line (spread) / game total only -- F5, NRFI/YRFI,
    # and pitcher_strikeouts are out of scope (see core/market_gate.py).
    "MLB": frozenset({
        "moneyline",
        "run_line",
        "game_total",
...
MARKET_PRIORITY: dict[tuple[str, str], int] = {
    ("MLB",  "moneyline"):          1,   # full-game moneyline -- newly in scope
