...
def get_recent_k_bf_splits(pitcher_id: int | None, season: int, n: int = 8) -> dict | None:
    ...
    Returns None when fewer than 4 real starts have usable battersFaced /
    ...
    than admitting the data isn't there yet; the caller should fall back
    to its existing projection path in that case, not fabricate one here.

    Returns dict with:
        n_starts    -- real starts used
        k_pct_mean  -- mean per-start K% (strikeouts / batters_faced)
    ...
    k_std  = statistics.stdev(k_pcts) if len(k_pcts) > 1 else max(0.02, k_mean * 0.15)

    return {
        "n_starts":   len(bfs),
        "k_pct_mean": round(k_mean, 4),
