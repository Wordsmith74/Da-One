    ...
    for event in events:
        for comp in event.get("competitions", []):
            for competitor in comp.get("competitors", []):
                ...
                # Exact match on any field, OR a substring match -- but only
                # allow substring matching once the query is long enough
                # (>3 chars) to avoid a 2-3 letter abbreviation spuriously
                # matching inside an unrelated longer string.
                if any(team_query == c for c in candidates) or (
                    len(team_query) > 3 and any(team_query in c for c in candidates)
                ):
