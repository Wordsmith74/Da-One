...
from core.intelligence.stat_model import get_stat_model_factor
from core.intelligence.rivalry_intel import get_rivalry_intel
...
def _get_team_histories(
    ...
    AUDIT (2026-08-05): no longer falls back to _synthetic_history(). That
    fallback centered fabricated data exactly at the sport's league average
    and fed it through NUTS to manufacture a posterior/confidence score --
    the same fabrication pattern found and removed from pitcher_strikeouts
    and WNBA player_rebounds/assists. Real game logs or an empty history
    (caller must skip the market) -- never invented data standing in as
    if it were real.
    """
    from core.odds_client import _abbrev  # type: ignore[attr-defined]
    ...
    # No synthetic backfill: if either side's real history is unusable, the
    # caller (build_expanded_game_candidates, around the _get_team_histories
    # call site) checks for empty lists and skips this game/market entirely
    # rather than pricing off invented data.
...
def _process_team_total(
    ...
    for side, hist, abbr, team_name in (
        ...
        # ── stat_model.py wiring (MLB team totals only) ─────────────────────
        # get_stat_model_factor().edge_adjustment was computed but discarded
        # everywhere it was called (only .pace ever got copied onto a
        # candidate, as pace_projection metadata for the integrity filter).
        # It's dimensionally valid here specifically because this market
        # compares one team's own average scoring against a single-team
        # line -- exactly what the module computes -- unlike full game_total,
        # where a team's own average can't be compared against a *combined*
        # two-team line without double-counting. direction="over" is passed
        # unconditionally (same reasoning as venue_intel's wiring in
        # odds_client.py): the function's sign flips with the requested
        # direction, but eff_mean itself is direction-agnostic, so we want
        # the raw "team is trending above/below its own total line" signal,
        # not a side-flipped one. Uses the median posted over-line for this
        # team (if any book has quoted one yet) as the comparison point;
        # falls back to the running eff_mean when no book line exists yet.
        _stat_adj = 0.0
        if sport.upper() == "MLB":
            try:
                _over_pts = sorted(pt for pt, _, _ in team_lines[side]["over"])
                _stat_ref_line = _over_pts[len(_over_pts) // 2] if _over_pts else eff_mean
                _stat_factor = get_stat_model_factor(
                    abbr, "Team Total", sport,
                    sportsbook_line=_stat_ref_line, direction="over",
                )
                _stat_adj = _stat_factor.edge_adjustment or 0.0
            except Exception as exc:
                print(f"[game_markets] stat_model unavailable for {abbr}: {exc}", flush=True)
            if _stat_adj:
                eff_mean = round(eff_mean + _stat_adj, 2)
                print(
                    f"[game_markets] {abbr} team-total stat_model adj: "
                    f"{_stat_adj:+.2f} -> mean={eff_mean}", flush=True,
                )
...
def _process_spread(
    ...
    # Stale line guard thresholds (used both for the cross-side pairing
    # check below and the same-side consensus check further down).
    # Mirrors the guard in core/odds_client.py's total_over/total_under
    # handling.
    ...
    # ── Cross-side pairing guard (2026-07-08 fix) ──────────────────────────
    # Previously home_lines/away_lines were built as two independent flat
    # lists, then _best_side() picked the best-odds book for each side with
    # NO check that the two picks actually came from books agreeing with
    # each other. Since a run line's home and away points must mirror
    # (-1.5 / +1.5), but nothing enforced that, it was possible for the
    # published pick to pair a stale/off-market book on one side with a
    # current book on the other -- e.g. MIN home -1.5 (from a book that
    # hadn't refreshed) alongside CLE away +1.0 (from a book that had),
    # producing a candidate with a spread sign/magnitude that didn't match
    # the live market on either side.
    #
    # Fix: first group outcomes by book so each book's home+away quote is
    # evaluated as a pair (they come from the same API bookmaker snapshot,
    # so pairing them back up is safe). A book only contributes to
    # home_lines/away_lines if BOTH its home and away points are present
    # AND they mirror each other within the drift tolerance. Books with
    # mismatched home/away pairs (internally inconsistent -- a stale feed)
    # are dropped entirely rather than letting one side of them leak into
    # the pool.
    _book_sides: dict[str, dict[str, tuple[float, int]]] = {}
    for bk_title, outcomes in book_outcomes_list:
        for out in outcomes:
            name  = out.get("name") or ""
            price = out.get("price")
            ...
            if price is None or pt is None:
                continue
            if name == home_team:
                _book_sides.setdefault(bk_title, {})["home"] = (float(pt), int(price))
            elif name == away_team:
                _book_sides.setdefault(bk_title, {})["away"] = (float(pt), int(price))

    home_lines: list[tuple[float, int, str]] = []
    away_lines: list[tuple[float, int, str]] = []

    for bk_title, sides in _book_sides.items():
        home_side = sides.get("home")
        away_side = sides.get("away")
        if home_side is None or away_side is None:
            continue  # book only quoted one side -- can't validate, skip it
        home_pt, home_price = home_side
        away_pt, away_price = away_side
        if abs(home_pt + away_pt) > _drift_limit:
            print(
                f"[game_markets] MISMATCHED BOOK dropped: {away_team}@{home_team} "
                f"{bk_title} home={home_pt:+g} away={away_pt:+g} "
                f"(don't mirror within {_drift_limit})",
                flush=True,
            )
            continue
        home_lines.append((home_pt, home_price, bk_title))
        away_lines.append((away_pt, away_price, bk_title))
    ...
    # the in-consensus books first. Still needed even with the pairing
    # guard above: it catches the case where a *majority* of books are
    # internally-consistent-but-stale together (rare, but not impossible
    # during a fast-moving market), which pairing alone can't detect.

    # rivalry_intel.py wiring (MLB spread/run-line) -- same volatility-shrink
    # treatment as _process_moneyline above; see that function for the full
    # rationale. Computed once per game since it's a game-level signal.
    _rivalry_shrink = 0.0
    if sport.upper() == "MLB":
        try:
            _rivalry = get_rivalry_intel(home_team, away_team, sport, home_abbr, away_abbr)
    ...
    for side, lines, team_abbr, team_name in (
        ...
        if lines:
            # True median (2026-07-08 fix) — the old `_all_pts[len//2]` only
            # works for odd-length lists. On an even-length list it silently
            # returns the *upper*-middle element instead of averaging the two
            # middle values, which breaks ties toward whichever point value
            # happens to sort higher. That matters a lot here: a line move
            # in progress often looks like exactly half the books still on
            # the old number and half already on the new one, and the old
            # indexing would deterministically side with the higher number
            # every time regardless of which one is actually current.
            _pt_values = [pt for pt, _, _ in lines]
            _consensus_line = statistics.median(_pt_values)

            # If the book pool is an even split between exactly two distinct
            # points, there's no real "consensus" to check against -- it's a
            # coin flip which one is the live number from this snapshot
            # alone. Publishing either would be a guess, so skip the side
            # entirely rather than risk shipping the stale one.
            _distinct = sorted(set(_pt_values))
            if len(_distinct) == 2:
                _low_n  = _pt_values.count(_distinct[0])
                _high_n = _pt_values.count(_distinct[1])
                if _low_n == _high_n:
                    print(
                        f"[game_markets] AMBIGUOUS LINE skipped: {away_team}@{home_team} "
                        f"{side} split evenly between {_distinct[0]:+g} and {_distinct[1]:+g} "
                        f"({_low_n} books each) -- no clear consensus this snapshot",
                        flush=True,
                    )
                    continue
        ...
        if _rivalry_shrink:
            model_prob = 0.5 + (model_prob - 0.5) * (1.0 - _rivalry_shrink)
...
def fetch_expanded_game_candidates(
    ...
    for game in raw_games:
        ...
        if not home_hist or not away_hist:
            # AUDIT (2026-08-05): previously _get_team_histories() silently
            # backfilled whichever side was missing with fabricated data
            # centered at the league average, so this game/market always
            # produced a candidate even with zero real signal. No real game
            # log for either team now means NO PICK for this game across
            # moneyline/totals/run_line (they all share comb_hist below),
            # not a confident-looking number built on invented data.
            print(
                f"[game_markets] NO PLAY: {game_label} — real team game-log "
                "history unavailable for moneyline/totals/run_line "
                "(synthetic fallback disabled)", flush=True,
            )
            continue
