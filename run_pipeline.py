...
def live_fetch_mlb_f5_matchup(game, season):
    ...
    game_odds = next((o for o in odds_data if home_name in o.get("home_team", "")), None)

    if not game_odds:
        print(f"  [info] No F5 odds resolved for {away_name} @ {home_name} -- skipping this matchup")
        return None

    return {
        "matchup": f"{away_name} @ {home_name}",
        "home_recent_runs": [],   # TODO: derive from get_mlb_player_game_logs aggregated to team level,
        "away_recent_runs": [],   # or a team-level stats endpoint if balldontlie exposes one
        "home_opp_pitcher_siera": None,  # TODO: resolve probable starter -> get_savant_pitcher_advanced_stats
        "away_opp_pitcher_siera": None,
        "park_factor_full_game": park_factor,
        "market_f5_total_line": None,    # TODO: parse from game_odds once F5 market key is confirmed
...
def live_fetch_mlb_pitcher_k_prop(pitcher_player_id, pitcher_name, opponent_team_id, season):
    ...
    return {
        "matchup": "TBD",  # fill from game lookup
        ...
        "market_k_line": None, "market_k_odds": -110,  # TODO: from props once event_id wiring is added
    }
...
def live_fetch_wnba_player_prop(player_id, season):
    ...
    return {
        ...
        "status_history": None,  # TODO: wire injury status if available
        "market_pts_line": None, "market_pts_odds": -110,  # TODO: from props endpoint
    }
...
def run_pipeline():
    ...
    # ---- MLB K props / WNBA props: still mock-only ----
    # Honest gap: these need a resolved "which pitchers/players are playing today"
    # roster step (probable starters, today's active lineups) that isn't built yet --
    # going live here would mean guessing which player IDs to even ask the API about.
    # Safer to stay on mock for these two until that roster-resolution step exists,
    # rather than silently produce picks for the wrong/stale player.
    for label, process_fn, mock_fn in [
        ("MLB K prop", process_mlb_k_prop, mock_fetch_mlb_pitcher_k_prop),
        ("WNBA prop", process_wnba_prop, mock_fetch_wnba_player_prop),
    ]:
        if USE_LIVE_DATA:
            log("warn", label, "live roster-resolution not yet built -- using mock data (see SETUP.md)")
        try:
            raw = mock_fn()
            pick = process_fn(raw)
            if pick:
                raw_picks.append(pick)
                log("info", label, f"generated pick: {pick['pick']} (edge {pick['edge_pct']}%)")
            else:
                log("info", label, "no edge cleared threshold -- skipped")
        except Exception as e:
            log("error", label, f"failed ({type(e).__name__}: {e}) -- skipping this market entirely")
