...
# ---------------------------------------------------------------------------
# Market-specific tier overrides, split by side (over/under)
# ---------------------------------------------------------------------------
# pitcher_strikeouts over vs. under have historically graded very differently
# (shadow_log_graded.jsonl, 775 graded MLB-K shadow-log rejects, graded
# 2026-07-25): over 195-169 (53.6% win), under 161-238 (40.4% win) -- a 13pt
# gap the pooled table above cannot see. Side-split grid search over that
# same 775-record pool (breakeven 52.38% @ -110, n>=15 floor per
# threshold_optimizer.py convention):
#   over:  edge>=1.0%, conf>=60.0 -> n=135, 77W-58L, win%=57.0%, roi=+8.9%
#   under: edge>=3.0%, conf>=65.0 -> n=32,  21W-11L, win%=65.6%, roi=+25.3%
# (Grid tested edge in {0,0.5,1,...,5}, conf in {55,60,...,85}; values above
# are each side's best-ROI cutoff clearing breakeven at n>=15.)
#
# Only GOLD is replaced here, with the exact cutoffs above. NUKE/DIAMOND are
# carried over unchanged from the pooled table's 2026-07-07 n=99 spec --
# there isn't yet a graded, side-split sample at that higher edge/confidence
# band to re-derive them from (the 775-record pool above is almost entirely
# BELOW the old pooled Gold bar, since it's a reject/shadow log by
# construction). Re-run core/threshold_optimizer.py with side as a split
# dimension once more data clears the current entry floor and re-derive
# Nuke/Diamond per side at that point -- don't hand-tune them without it.
_MARKET_TIER_THRESHOLDS_BY_SIDE: dict[tuple[str, str], list[tuple[Tier, float, float]]] = {
...
_MARKET_ENTRY_FLOORS: dict[str, tuple[float, float]] = {
    "pitcher_strikeouts": (3.0, 70.0),   # MLB Ks — FALLBACK ONLY, used when bet.direction is
                                          # missing/unrecognized. Normal traffic is gated by
                                          # _MARKET_ENTRY_FLOORS_BY_SIDE below instead (see that
                                          # dict's comment for the evidence). This value is the
                                          # prior side-agnostic floor (2026-07-07 spec), kept as
                                          # a conservative default rather than removed outright.
...
# ---------------------------------------------------------------------------
# Market-specific entry floors, split by side (over/under)
# ---------------------------------------------------------------------------
# Checked BEFORE _MARKET_ENTRY_FLOORS_BY_SPORT / _MARKET_ENTRY_FLOORS in Step 0.5
# below. Same evidence as _MARKET_TIER_THRESHOLDS_BY_SIDE above (775-record
# shadow-log pool, graded 2026-07-25) -- set equal to that table's new Gold
# cutoffs per side so the entry floor never sits below the tier it's supposed
# to gate (the old pooled floor of 3.0/70.0 already had this exact bug once,
# see _MARKET_ENTRY_FLOORS's comment above).
_MARKET_ENTRY_FLOORS_BY_SIDE: dict[tuple[str, str], tuple[float, float]] = {
    ("pitcher_strikeouts", "over"):  (1.0, 60.0),
    ("pitcher_strikeouts", "under"): (3.0, 65.0),
}

# Full-game run line / spread / total floors, split per sport instead of one shared
# "run_line" entry.
#
# MLB moneyline / run_line / game_total are OUT OF SCOPE -- removed entirely, not just
# floored high. See core/market_gate.py's ALLOWED_MARKETS["MLB"] (the authoritative
# scope definition -- candidates for these three markets are blocked before they ever
# reach simulation, so they can never reach this table regardless of what's in it).
# This dict previously carried provisional (4.0, 60.0) / (2.53, 75.0) floors for those
# three markets from back when they were still in scope on unproven, single-digit-n
# samples. output/threshold_recommendations.json (generated 2026-08-04, n=31 graded)
# has since confirmed the call: MLB market_class=game -> status="insufficient_data" --
# no edge/confidence cutoff across all three combined cleared breakeven ROI at n >= 15.
# Real graded results by then: moneyline 35.3% win/-0.52 units (n=34), run_line 25.0%
# win/-0.16 units (n=12), game_total's own tiny n=2 sample never actually generalized.
# Entries removed rather than left as unreachable dead weight so this table can't
# silently start gating live traffic again if market_gate.py's scope is ever loosened
# without someone also revisiting this file.
_MARKET_ENTRY_FLOORS_BY_SPORT: dict[str, dict[str, tuple[float, float]]] = {
    "WNBA": {
        ...
        "moneyline": (4.7, 0.0),   # full-game moneyline -- lower bound of the requested
                                    # 4.7-8.2% edge band (upper bound enforced separately as
                                    # a ceiling, see _MARKET_MAX_EDGE_BY_SPORT below). No
                                    # confidence floor set here (0.0 = no-op) since the
                                    # request was edge-only; the sport-wide WNBA tier
                                    # thresholds still apply confidence gating downstream.
        "player_rebounds": (4.0, 60.0),   # lower bound of the requested 4-8% edge band
                                    # (upper bound enforced separately -- edge ceiling in
                                    # _MARKET_MAX_EDGE_BY_SPORT below). Confidence floor
                                    # lowered 68.0 -> 60.0 per explicit request.
                                    # SAMPLE SIZE CAVEAT: derived from just 5 distinct graded
                                    # shadow-log rejects that fall inside the prior 4.3-8.5%
                                    # band (2026-07-06..12), and all 5 happened to win. That's
                                    # far below threshold_optimizer.py's own 15-graded-pick
                                    # minimum for a supported threshold (see MLB game_total's
                                    # note above, and Step 0.5b below, which already declined
                                    # to set an edge/confidence threshold for this same market
                                    # off a LARGER n=18 sample for exactly this reason). A
                                    # 5-for-5 band this narrow is also exactly what you'd
                                    # expect from fitting bounds to a small sample after the
                                    # fact, not necessarily a real effect. Re-derive once
                                    # player_rebounds has >=15 graded picks inside this band
                                    # either way.
    },
}

# ── Market edge ceilings ────────────────────────────────────────────────────
# Mirror of _MARKET_ENTRY_FLOORS_BY_SPORT above, but as an UPPER bound: a bet
# whose edge_percentage exceeds the ceiling is discarded rather than treated
# as an extra-strong signal. Exists for markets where a very high computed
# edge is more likely to indicate a bad price / stale line / model glitch
# than genuine value -- unlike the entry floors, there is no sport-agnostic
# fallback table, since a missing entry here means "no ceiling" by design.
_MARKET_MAX_EDGE_BY_SPORT: dict[str, dict[str, float]] = {
    "WNBA": {
        # Both graded WNBA moneyline picks with edge >= 10% (GSV@TOR, IND@LAS,
        # both large plus-money underdog prices) lost, while every graded pick
        # in the 4.7-8.2% range won -- see 2026-07-07..09 graded sample. Reject
        # rather than trust an edge reading this high on WNBA moneyline until
        # there's a graded sample confirming it's signal and not noise from the
        # precomputed (non-Bayesian) moneyline model on thin plus-money prices.
        # Paired with the 4.7% floor in _MARKET_ENTRY_FLOORS_BY_SPORT above,
        # this pins WNBA moneyline to the 4.7-8.2% band per explicit request.
        "moneyline": 8.2,
        # Upper bound of the 4-8% player_rebounds band -- see the floor's
        # comment in _MARKET_ENTRY_FLOORS_BY_SPORT above for the same n=5
        # sample-size caveat; this ceiling is exactly as provisional as that
        # floor and should be re-derived alongside it.
        "player_rebounds": 8.0,
    },
}

# ── Market confidence ceilings ──────────────────────────────────────────────
# Same idea as _MARKET_MAX_EDGE_BY_SPORT above, but for confidence_score
# rather than edge_percentage. No sport-agnostic fallback -- a missing entry
# means "no ceiling" by design.
# This table has only ever held one entry: WNBA player_rebounds (70.2),
# removed per explicit request. No other sport/market has ever had a
# confidence ceiling here, so there is nothing else to preserve -- this
# dict is empty rather than "WNBA player_rebounds uncapped among others."
_MARKET_MAX_CONF_BY_SPORT: dict[str, dict[str, float]] = {}
...
# Mirrors models/injury_intel.py's _LATE_UNCONFIRMED_WINDOW_HOURS -- kept as
# a separate constant here rather than importing the private one, since this
# is purely about how Step 0.7 logs a visibility note (not a discard), and
# the two modules shouldn't share a private threshold across a package
# boundary. If you change one, change both.
_LATE_UNCONFIRMED_WINDOW_HOURS_GK = 1.5
...
def evaluate_tier(
    ...
    entirely for this call. If `direction` ("over"/"under") is given and a
    side-split override exists in _MARKET_TIER_THRESHOLDS_BY_SIDE for this
    market+side, that takes priority over the pooled per-market table.
    ...
    if market is not None:
        mkt_norm = market_normalized(market)
        if direction is not None:
            thresholds = _MARKET_TIER_THRESHOLDS_BY_SIDE.get((mkt_norm, direction.strip().lower()))
        if thresholds is None:
            thresholds = _MARKET_TIER_THRESHOLDS.get(mkt_norm)
    if thresholds is None:
...
def stamp_tier(bet: Bet, sport: str = _DEFAULT_SPORT_KEY) -> Bet:
    ...
    """
    bet.tier = evaluate_tier(bet.edge_percentage, bet.confidence_score, sport, bet.market, bet.direction)
    return bet
...
def check_for_conflicts(list_of_bets: list[Bet], sport: str = _DEFAULT_SPORT_KEY) -> list[Bet]:
    ...
    for i in range(n):
        for j in range(i + 1, n):
            ...
            lower.tier = evaluate_tier(lower.edge_percentage, lower.confidence_score, sport, lower.market, lower.direction)
    ...
    for (_gid, _mn), _grp in _gm_groups.items():
        ...
        for _loser in _all_sides:
            ...
            _loser.tier = evaluate_tier(_loser.edge_percentage, _loser.confidence_score, sport, _loser.market, _loser.direction)
    ...
    for game_bets in by_game.values():
        ...
        for tt in team_totals:
            ...
            if ratio >= _PROP_CONSENSUS_THRESHOLD:
                ...
                tt.tier = evaluate_tier(tt.edge_percentage, tt.confidence_score, sport, tt.market, tt.direction)
...
def run_gatekeeper(bets: list[Bet], sport: str = _DEFAULT_SPORT_KEY) -> dict[str, list[Bet]]:
    ...
    for bet in bets:
        ...
        # Side-split floor takes priority over everything else when the market has
        # one (currently just pitcher_strikeouts -- see _MARKET_ENTRY_FLOORS_BY_SIDE's
        # comment for the evidence). Sport-specific floor is next (e.g. MLB run_line
        # vs WNBA run_line are no longer the same entry) -- fall back to the
        # sport-agnostic table only for markets that are inherently single-sport
        # already (Ks, F5 markets, NRFI/YRFI, assists, etc.)
        floor = _MARKET_ENTRY_FLOORS_BY_SIDE.get((mkt_norm, (bet.direction or "").strip().lower()))
        if floor is None:
    ...
    # ── Step 0.5b: WNBA assists/rebounds hard risk gates ───────────────────────
    # Two additional hard entry gates for player_assists / player_rebounds,
    # on top of Step 0.5's edge/confidence floor. Both come from a
    # reject-log review of graded near-miss picks (2026-07-06..10, n=18
    # resolved -- see data/bet_rejects.jsonl): edge and confidence in the
    # 55-68 conf / 0-6% edge band showed NO separation between wins and
    # losses (win-group avg conf 62.6 / edge 2.98%; loss-group avg conf
    # 61.2% / edge 3.32% -- statistically indistinguishable, n too small
    # for a supported threshold change either way per
    # threshold_optimizer.py's own 15-graded-pick minimum). Two things DID
    # separate the groups:
    #
    #   1. minutes_range >= 10: of the 4 largest L5 minutes-range values in
    #      the whole sample (Cloud 16, Burton 13, Williams 11, Thomas 10),
    #      all 4 lost; the existing Step 1b2 stability cap only shrinks
    #      stake size for these, it doesn't stop them being bet. This
    #      raises that to a hard discard specifically for the market where
    #      the pattern showed up.
    #   2. ramp_flag (core/player_props.py's raw_result["ramp_flag"]) --
    #      returning-from-injury / workload-still-ramping players. Assist
    #      totals depend on court time, and a player mid-ramp has a
    #      materially less reliable minutes baseline than the model's
    #      season/L5 average assumes, independent of what edge/confidence
    #      it computed.
    #
    # NOTE: no edge ceiling is added here despite 3 of the top-4 edges in
    # the same sample being losses (Thomas 5.9%, Burton 5.9%, Shepard
    # 5.1%) -- n=4 is far below the file's own evidentiary bar for a new
    # threshold (see _MARKET_MAX_EDGE_BY_SPORT's WNBA moneyline comment).
    # Flagged as a watch item: re-evaluate once player_assists has >=15
    # graded picks above 5% edge.
    _WNBA_RISK_GATE_MKTS = {"player_assists", "player_rebounds"}
    _MINUTES_RANGE_HARD_GATE = 10.0
    for bet in bets:
        if bet.tier is None or bet.flagged:
            continue
        mkt_norm = market_normalized(bet.market)
        if mkt_norm not in _WNBA_RISK_GATE_MKTS:
            continue

        _mrange = bet.raw_result.get("minutes_range")
        _ramp   = bet.raw_result.get("ramp_flag")

        gate_reasons: list[str] = []
        if isinstance(_mrange, (int, float)) and _mrange >= _MINUTES_RANGE_HARD_GATE:
            gate_reasons.append(
                f"L5 minutes_range {_mrange:.0f}min >= {_MINUTES_RANGE_HARD_GATE:.0f}min hard gate"
            )
        if _ramp:
            gate_reasons.append("returning-from-injury / workload-ramp flag")

        if gate_reasons:
            reason = (
                f"WNBA risk gate [{mkt_norm}]: " + "; ".join(gate_reasons)
            )
            bet.tier        = None
            bet.flagged     = True
            bet.flag_reason = _append_flag(bet.flag_reason, reason)
            logger.debug(f"[gatekeeper] RISK_GATE {bet.bet_id}: {reason}")
            import datetime as _dt_risk
            _log_reject(bet, sport, _dt_risk.date.today().isoformat(),
                        "wnba_risk_gate", reason_override=reason)

    # ── Step 0.5c: MLB Ks confidence-band discard ───────────────────────────
    # confidence_score for this market is derived in run_pipeline.py's
    # _derive_bet_params() as 50 + z*25 (z = |posterior_mean - line| /
    # max(posterior_std, sigma_floor)), then Step 0 above adds a flat +5pts
    # market-weight bump for Tier-1 props. That's a purely distributional
    # score, not something validated against real hit rate -- and for this
    # market it is NOT monotonic with outcome: shadow_log_graded.jsonl (775
    # graded MLB-K shadow-log records, graded 2026-07-25) shows
    #   conf 60-65: 214 picks, 48.6% win
    #   conf 65-70: 131 picks, 55.0% win
    #   conf 70-78: 133 picks, 34.6% win   <- worse than every band below it
    #   conf 78-85:  36 picks, 38.9% win
    #   conf 85+:    77 picks, 46.8% win
    # 34.6% is well under the -110 breakeven of 52.38%, and the dip isn't a
    # side artifact (over 38.9% / under 29.5% in-band, both bad). Root cause
    # is unconfirmed -- plausibly the z-score-to-confidence mapping and/or
    # the +5 prop bump systematically over-stating confidence for picks
    # whose true edge sits in a particular sigma/z range for this market --
    # but the pattern itself is clear and reproducible at n=133, well above
    # threshold_optimizer.py's 15-graded-pick minimum. Hard-discard rather
    # than demote until the root cause in the simulation/calibration layer
    # is found: a confidence number this specifically wrong is worse than no
    # signal at all, because it reads as MORE trustworthy than the bands
    # around it. Re-open once posterior_mean/posterior_std are logged
    # per-pick (they currently are not persisted to the shadow log) and the
    # z-range driving this band can be isolated directly.
    _MLB_K_CONF_DISCARD_MKTS = {"pitcher_strikeouts"}
    _MLB_K_CONF_BAND = (70.0, 78.0)
    for bet in bets:
        if bet.tier is None or bet.flagged:
            continue
        if sport.upper() != "MLB":
            continue
        mkt_norm = market_normalized(bet.market)
        if mkt_norm not in _MLB_K_CONF_DISCARD_MKTS:
            continue
        lo, hi = _MLB_K_CONF_BAND
        if lo <= bet.confidence_score < hi:
            reason = (
                f"MLB Ks confidence-band discard [{mkt_norm}]: conf "
                f"{bet.confidence_score:.1f} in [{lo:.0f}, {hi:.0f}) -- "
                f"this band graded 34.6% win (n=133, 2026-07-25 shadow log), "
                f"below every neighboring band and below -110 breakeven"
            )
            bet.tier        = None
            bet.flagged     = True
            bet.flag_reason = _append_flag(bet.flag_reason, reason)
            logger.debug(f"[gatekeeper] CONF_BAND {bet.bet_id}: {reason}")
            import datetime as _dt_confband
            _log_reject(bet, sport, _dt_confband.date.today().isoformat(),
                        "mlb_k_confidence_band", reason_override=reason)

    # ── Step 0.7: Pregame availability check (MLB player props) ─────────────
    # Uses models/injury_intel.check_pregame_availability() -- see that
    # function's docstring for the full tri-state contract. The short
    # version, because it matters for how this step is written: lineups and
    # last-minute starter scratches often aren't posted until 1-2 hours
    # before first pitch, so "no confirmation yet" is the NORMAL state for
    # most of a pipeline run's day, not a red flag. Only "confirmed_out" is
    # a hard signal here -- "unconfirmed" (even the late-and-still-quiet
    # variant) is logged for visibility but never discards or penalizes a
    # bet on its own. Expects bet.raw_result to carry "injury_data"
    # (pre-fetched, same shape as data/rotowire_injuries_mlb's return) and
    # optionally "game_time_utc" -- bets without either simply get an
    # "unknown" status and pass through untouched (fail-open, same
    # contract as compute_injury_adjustment()).
    _PREGAME_CHECK_MKTS = {"pitcher_strikeouts"}
    if _PREGAME_AVAILABILITY_AVAILABLE and sport.upper() == "MLB":
        for bet in bets:
            if bet.tier is None or bet.flagged:
                continue
            mkt_norm = market_normalized(bet.market)
            if mkt_norm not in _PREGAME_CHECK_MKTS or not bet.player:
                continue
            injury_data = bet.raw_result.get("injury_data")
            game_time_utc = bet.raw_result.get("game_time_utc")
            try:
                result = _check_pregame_availability(
                    bet.player, bet.team, game_time_utc, injury_data,
                )
            except Exception as exc:  # fail-open -- never let this step crash a run
                logger.debug(f"[gatekeeper] PREGAME_CHECK exception for {bet.bet_id}: {exc}")
                continue

            if result["status"] == "confirmed_out":
                reason = f"Pregame availability [{mkt_norm}]: {result['reason']}"
                bet.tier        = None
                bet.flagged     = True
                bet.flag_reason = _append_flag(bet.flag_reason, reason)
                logger.debug(f"[gatekeeper] PREGAME_OUT {bet.bet_id}: {reason}")
                import datetime as _dt_pregame
                _log_reject(bet, sport, _dt_pregame.date.today().isoformat(),
                            "pregame_availability", reason_override=reason)
            elif result["status"] == "unconfirmed" and result.get("hours_to_game") is not None \
                    and result["hours_to_game"] <= _LATE_UNCONFIRMED_WINDOW_HOURS_GK:
                # Visibility only -- NOT a discard, NOT a confidence penalty.
                # See Step 0.7's docstring above for why late-unconfirmed
                # must not be treated as a negative signal on its own.
                note = f"Pregame availability [{mkt_norm}]: {result['reason']}"
                bet.flag_reason = _append_flag(bet.flag_reason, note)
                logger.debug(f"[gatekeeper] PREGAME_LATE_UNCONFIRMED {bet.bet_id}: {note}")

    # ── Step 0.6: Market-specific edge ceilings ────────────────────────────────
    # Opposite of Step 0.5's floor: some markets discard a bet whose edge is
    # TOO high rather than too low (see _MARKET_MAX_EDGE_BY_SPORT). Runs after
    # the floor step and skips anything the floor already discarded.
    for bet in bets:
        if bet.tier is None and bet.flagged:
            continue   # already discarded in Step 0.5
        mkt_norm = market_normalized(bet.market)
        ceiling = _MARKET_MAX_EDGE_BY_SPORT.get(sport.upper(), {}).get(mkt_norm)
        if ceiling is None:
            continue   # no ceiling for this sport/market → uncapped
        if bet.edge_percentage > ceiling:
            reason = (
                f"Market edge ceiling [{mkt_norm}]: edge {bet.edge_percentage:.2f}% "
                f"> ceiling {ceiling:.1f}%"
            )
            bet.tier        = None
            bet.flagged     = True
            bet.flag_reason = _append_flag(bet.flag_reason, reason)
            logger.debug(f"[gatekeeper] CEILING  {bet.bet_id}: {reason}")
            import datetime as _dt_ceiling
            _log_reject(bet, sport, _dt_ceiling.date.today().isoformat(),
                        "market_edge_ceiling", reason_override=reason)

    # ── Step 0.6b: Market-specific confidence ceilings ─────────────────────────
    # Same shape as Step 0.6, but for confidence_score (see
    # _MARKET_MAX_CONF_BY_SPORT). Runs after the edge ceiling step and skips
    # anything already discarded by an earlier step.
    for bet in bets:
        if bet.tier is None and bet.flagged:
            continue   # already discarded in an earlier step
        mkt_norm = market_normalized(bet.market)
        conf_ceiling = _MARKET_MAX_CONF_BY_SPORT.get(sport.upper(), {}).get(mkt_norm)
        if conf_ceiling is None:
            continue   # no confidence ceiling for this sport/market → uncapped
        if bet.confidence_score > conf_ceiling:
            reason = (
                f"Market confidence ceiling [{mkt_norm}]: conf {bet.confidence_score:.1f} "
                f"> ceiling {conf_ceiling:.1f}"
            )
            bet.tier        = None
            bet.flagged     = True
            bet.flag_reason = _append_flag(bet.flag_reason, reason)
            logger.debug(f"[gatekeeper] CONF_CEILING  {bet.bet_id}: {reason}")
            import datetime as _dt_conf_ceiling
            _log_reject(bet, sport, _dt_conf_ceiling.date.today().isoformat(),
                        "market_confidence_ceiling", reason_override=reason)
    ...
    # ── Step 1b4: MLB pitcher-strikeout OVER hook-risk penalty ────────────────
    # A starter can only rack up strikeouts for as long as he stays in the
    # game. When core/pitcher_workload.py's innings projection has LOW
    # confidence (model IP diverges meaningfully from the sportsbook-implied
    # IP, or too few workload components were available), an OVER pick is
    # exposed to real early-hook risk: the pitcher gets pulled before he can
    # reach the line. An UNDER pick is not symmetrically exposed -- an early
    # hook can only help an UNDER, never hurt it -- so this penalty applies to
    # OVER picks only.
    #
    # core/player_props.py already computes and stores this signal
    # (raw_result["workload_confidence_tier"]) specifically for this step (see
    # the "read by gatekeeper Step 1b4" comments there), but until now nothing
    # ever consumed it, so it was a dead signal and OVER picks on volatile
    # workload projections went through this file with no additional
    # scrutiny -- exactly the shape of a systematic OVER-side miss surfaced in
    # threshold-recommendation review (24 Gold-tier MLB-props losses skewed
    # toward "over": 52.9% over win% vs 64.0% under win%, in graded picks from
    # 2026-06-30 to 2026-07-11).
    #
    # Penalty (tier is re-evaluated afterward against the market's own
    # threshold table, same as Step 1b3):
    #   "Low"    workload confidence (score < 45, per _CONF_MEDIUM in
    #            core/pitcher_workload.py) → −12 confidence
    #   "Medium" workload confidence (45 ≤ score < 70)                → −5 confidence
    #   "High"   workload confidence, or tier missing/unknown         → no penalty
    # Applies ONLY to MLB pitcher_strikeouts OVER picks.
    _HOOK_RISK_CONF_PENALTY: dict[str, float] = {
        "Low":    12.0,
        "Medium":  5.0,
    }
    ...
    for bet in bets:
        ...
        if sport.upper() != "MLB":
            continue
        if market_normalized(bet.market) != "pitcher_strikeouts":
            continue
        if bet.direction.lower() != "over":
            continue

        workload_tier = bet.raw_result.get("workload_confidence_tier")
        penalty = _HOOK_RISK_CONF_PENALTY.get(workload_tier, 0.0)
        if penalty <= 0:
            continue
        ...
        bet.confidence_score = max(0.0, bet.confidence_score - penalty)
        reason = (
            f"MLB K OVER hook-risk [workload={workload_tier}]: "
            f"conf −{penalty:.0f}pts ({raw_conf:.1f}→{bet.confidence_score:.1f})"
        )
        bet.flag_reason = _append_flag(bet.flag_reason, reason)
        bet.tier = evaluate_tier(bet.edge_percentage, bet.confidence_score, sport, bet.market, bet.direction)
        if bet.tier is None:
            bet.flagged = True
        logger.debug(f"[gatekeeper] HOOK_RISK {bet.bet_id}: {reason}")
    ...
    # player_rebounds also gets its own lower floor now (60, lowered from 68
    # per explicit request) rather than the blanket 75 -- Step 0.5's entry
    # floor already enforces the 60.0 confidence floor for this market (see
    # _MARKET_ENTRY_FLOORS_BY_SPORT; the confidence ceiling that used to pair
    # with it has since been removed, so confidence is floored, not banded);
    # leaving a higher floor in place here would just re-reject bets this
    # step was meant to admit, one step later.
    # All other props use the standard floor (75).
    _PROP_CONF_FLOOR          = 75.0
    _PROP_CONF_FLOOR_OVERRIDES: dict[str, float] = {
        "strikeouts": 68.0,
        "player_rebounds": 60.0,
    }
    ...
    for bet in bets:
        ...
        floor = _PROP_CONF_FLOOR
        for key, override_floor in _PROP_CONF_FLOOR_OVERRIDES.items():
            if key in mkt:
    ...
    #
    # DEMOTE rather than drop: a bet that already cleared a legitimate tier's
    # base edge+confidence bar shouldn't be discarded outright just because it
    # falls a few points short of the NEXT tier up's plus-money floor -- that
    # treats "87.4 vs 88" the same as "no edge at all." Instead, step it down
    # tier-by-tier until it clears a plus-money floor it CAN meet (edge is
    # never the issue here: a bet that qualified for a higher tier already
    # exceeds every lower tier's edge requirement). Only fails outright if it
    # can't even clear Gold Standard's plus-money floor. Mirrors the
    # market-concentration-cap demotion above (tier change + flag_reason,
    # no bet.flagged=True -- stays in the 'approved' bucket).
    _TIER_ORDER: list[Tier] = [Tier.NUKE, Tier.DIAMOND, Tier.GOLD]
    _PLUS_MONEY_CONF_FLOORS: dict[Tier, float] = {
        Tier.NUKE:    88.0,
        Tier.DIAMOND: 81.0,
        Tier.GOLD:    71.0,
    }
    ...
    for bet in bets:
        ...
        if bet.tier not in _PLUS_MONEY_CONF_FLOORS:
            continue

        original_tier = bet.tier
        start_idx = _TIER_ORDER.index(original_tier)
        landed_tier = None
        for tier in _TIER_ORDER[start_idx:]:
            if bet.confidence_score >= _PLUS_MONEY_CONF_FLOORS[tier]:
                landed_tier = tier
                break

        if landed_tier is None:
            # Doesn't even clear Gold Standard's plus-money floor -- discard.
            reason = (
                f"Plus-money confidence floor: odds={bet.american_odds:+.0f} "
                f"requires conf ≥ {_PLUS_MONEY_CONF_FLOORS[Tier.GOLD]:.0f} "
                f"even at Gold Standard, got {bet.confidence_score:.1f}"
            )
            bet.flagged     = True
            bet.flag_reason = _append_flag(bet.flag_reason, reason)
            bet.tier        = None
        elif landed_tier != original_tier:
            reason = (
                f"Plus-money demotion: odds={bet.american_odds:+.0f} conf={bet.confidence_score:.1f} "
                f"insufficient for {original_tier.value} (needs ≥{_PLUS_MONEY_CONF_FLOORS[original_tier]:.0f}) "
                f"-- demoted to {landed_tier.value} (needs ≥{_PLUS_MONEY_CONF_FLOORS[landed_tier]:.0f})"
            )
            bet.tier        = landed_tier
            bet.flag_reason = _append_flag(bet.flag_reason, reason)
        # else: confidence clears the original tier's plus-money floor — no change.
