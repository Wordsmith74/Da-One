...
MLB = {
    ...
    "max_picks_per_day": None,         # uncapped -- every pick that clears the gatekeeper/edge floors publishes.
                                        # Was 6. WNBA's cap is untouched (see WNBA block below); this is MLB-only.
    ...
    # See models/nrfi_handicapper.py for the full tiered framework this
    # feeds. CALIBRATED against the documented source: "72-75% of MLB first
    # innings are scoreless league-wide" (NRFI_YRFI_F5_Elite_Handicapping
    # _Reference.md, Baseline Rates).
    "nrfi_league_scoreless_pct": 0.735,   # midpoint of the 72-75% range -- the
                                            # prior every game-level adjustment
                                            # gets checked against before trusting
                                            # a big deviation (same "sanity check
                                            # against the prior" discipline as
                                            # f5_mean_projection_std above).
    # Derived, not independently tuned: combined_lambda = -ln(scoreless_pct).
    # Kept as an explicit constant (not computed inline at import time) so a
    # future re-calibration of nrfi_league_scoreless_pct doesn't silently
    # drift this without a matching backtest re-run -- same reasoning as the
    # prior_weight_* keys above being hand-set rather than derived.
    "nrfi_combined_lambda_baseline": 0.3070,   # -ln(0.735), rounded

    # Reliability gatekeeper (doc: "Reliability filter -- only trust
    # first-inning splits for pitchers with a meaningful sample (~50+ career
    # starts). Season debuts, first starts back from injury, or
    # opener/bullpen-game situations should be down-weighted or excluded.")
    "min_career_starts_for_nrfi_reliability": 50,

    # Pseudo-count (in games) for shrinking a team's recent first-inning
    # scoring rate toward the league baseline -- mirrors prior_weight_f5_games
    # but smaller: first-inning-only samples are noisier per game (a single
    # inning, not five), so a fresh small sample should be trusted even less
    # relative to its size than the F5 case is.
    "prior_weight_nrfi_games": 14,

    # Lambda (not runs) uncertainty per team for nrfi_edge_with_uncertainty --
    # scaled down from f5_mean_projection_std (0.35 runs) in proportion to
    # the much smaller mean itself (~0.15/team vs ~4.25/team for F5).
    "nrfi_mean_projection_std": 0.045,

    # Fraction of the full park run-factor effect that plausibly carries into
    # a single first inning. Set well below f5_park_scale (0.55) -- park
    # effects (HR/fly-ball carry) matter most over a full 9 innings of
    # fly-ball opportunities; one inning gets a small, mostly-wind/temperature
    # driven slice of that. Not separately backtested (no first-inning-level
    # park factor dataset in this repo yet) -- treat as a reasoned default,
    # re-tune via models/backtest.py once real first-inning outcome data is
    # collected under this key.
    "nrfi_park_scale": 0.12,

    # Tier weights for models/nrfi_handicapper.project_combined_first_inning_lambda
    # -- ordered per the reference doc's own tier priority (pitcher heaviest,
    # then lineup, then environment). Must sum to 1.0.
    "nrfi_tier_weight_pitcher":     0.55,
    "nrfi_tier_weight_lineup":      0.30,
    "nrfi_tier_weight_environment": 0.15,
}
