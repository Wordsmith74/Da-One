...
def get_wnba_team_injuries(team_abbr, roster_names=None):
    ...
    from data.rotowire_injuries_wnba import get_last_failure_reason as _rotowire_reason

    rotowire_reason = None
    try:
        data = _rotowire_get(team_abbr)
        if data:
            return data
        rotowire_reason = _rotowire_reason()
    except Exception as exc:
        rotowire_reason = f"raised {exc}"
    ...
    try:
        ...
        if data:
            return data
        print(f"  [warn] WNBA injuries ({team_abbr}): RotoWire -- {rotowire_reason}; ESPN returned no data")
    except Exception as exc:
        print(f"  [warn] WNBA injuries ({team_abbr}): RotoWire -- {rotowire_reason}; ESPN also failed: {exc}")
...
def get_mlb_team_injuries(team_abbr):
    ...
    from data.rotowire_injuries_mlb import get_last_failure_reason as _rotowire_mlb_reason
    ...
    try:
        ...
        if data:
            return data
        rotowire_reason = _rotowire_mlb_reason()
    except Exception as exc:
        rotowire_reason = f"raised {exc}"
    ...
    try:
        ...
        if data:
            return data
        print(f"  [warn] MLB injuries ({team_abbr}): RotoWire -- {rotowire_reason}; ESPN returned no data")
    except Exception as exc:
        print(f"  [warn] MLB injuries ({team_abbr}): RotoWire -- {rotowire_reason}; ESPN also failed: {exc}")
