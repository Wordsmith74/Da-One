...
⚠️ NOT WIRED IN — do not call from run_pipeline.py without changes.

run_pipeline.py already writes output/picks.json itself (see the bottom of
run_pipeline()), with the schema {"generated_at": ..., "picks": [...]}.
send_daily_picks() below writes a DIFFERENT, incompatible schema
({"date": ..., "sports": {...}} keyed by BetDisplay objects) to the SAME
path. Calling both in the same run means whichever runs second silently
clobbers the other's output with a shape nothing downstream expects.

This module dates from an earlier main.py + BetDisplay-per-sport
architecture (pre-run_pipeline.py). It's kept here as a reference for that
output contract, but wiring it in requires first deciding whether
run_pipeline.py's own writer should be replaced by this one (and every
downstream reader of picks.json updated to match), not just adding a call
to it alongside the existing writer.
