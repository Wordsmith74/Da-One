...
def get_wnba_player_search(name):
    ...
    against /wnba/v1/players?search=. Used by get_wnba_player_game_logs to
    resolve a player NAME (the only identifier discover_wnba_player_props has,
    since that discovery now runs entirely off The Odds API) to balldontlie's
    numeric player_id before calling /wnba/v1/stats, which 404s if player_ids[]
    is a name instead of a numeric ID.
    """
