...
_MARKET_ENTRY_FLOORS_BY_SPORT: dict[str, dict[str, tuple[float, float]]] = {
    ...
    "WNBA": {
        ...
        "player_rebounds": (4.3, 68.0),   # lower bound of a requested 4.3-8.5% edge /
                                    # 68.0-70.2% confidence band (upper bounds enforced
                                    # separately -- edge ceiling in _MARKET_MAX_EDGE_BY_SPORT
                                    # below, confidence ceiling in _MARKET_MAX_CONF_BY_SPORT).
                                    # SAMPLE SIZE CAVEAT: derived from just 5 distinct graded
                                    # shadow-log rejects that fall inside this exact band
                                    # (2026-07-06..12), and all 5 happened to win. That's
...
_MARKET_MAX_EDGE_BY_SPORT: dict[str, dict[str, float]] = {
    "WNBA": {
        ...
        "moneyline": 8.2,
        # Upper bound of the 4.3-8.5% player_rebounds band -- see the floor's
...
# rather than edge_percentage. Didn't exist before this table -- added
# specifically for WNBA player_rebounds (see its floor's comment in
# _MARKET_ENTRY_FLOORS_BY_SPORT for the n=5 sample-size caveat that applies
# equally here). No sport-agnostic fallback, same reasoning as the edge
# ceiling table: a missing entry means "no ceiling" by design.
_MARKET_MAX_CONF_BY_SPORT: dict[str, dict[str, float]] = {
    "WNBA": {
        "player_rebounds": 70.2,
    },
}
...
def run_gatekeeper(bets: list[Bet], sport: str = _DEFAULT_SPORT_KEY) -> dict[str, list[Bet]]:
    ...
    # player_rebounds also gets its own lower floor now (68, same as
    # strikeouts) rather than the blanket 75 -- Step 0.5's entry floor
    # already enforces the real 68.0-70.2 confidence band requested for this
    # market (see _MARKET_ENTRY_FLOORS_BY_SPORT / _MARKET_MAX_CONF_BY_SPORT
    # and their shared n=5 sample-size caveat); leaving the blanket 75 floor
    # in place here would just re-reject the entire 68-70.2 band this step
    # was meant to admit, one step later.
    # All other props use the standard floor (75).
    _PROP_CONF_FLOOR          = 75.0
    _PROP_CONF_FLOOR_OVERRIDES: dict[str, float] = {
        "strikeouts": 68.0,
        "player_rebounds": 68.0,
    }
